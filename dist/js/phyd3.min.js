...
a.sequences[0]&&a.sequences[0].domainArchitecture&&a.sequences[0].domainArchitecture.sequenceLength?a.sequences[0].domainArchitecture.sequenceLength:0});return d3.scale.linear().domain([0,d3.max(d)]).range([0,b])};phyd3.phylogram.build=function(d,b,a){function e(){d3.select(d).select("svg").remove();n=d3.select(d).insert("svg").attr("width",U+"px").attr("height",a.height+"px").attr("overflow","hidden").attr("version","1.1").attr("font-family","Open Sans").attr("xmlns","http://www.w3.org/2000/svg").call(Z);
...
a.domainWidth:0)}function oa(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";for(var e=!0,f=a.nodeHeight,k=d.x,g=d.y,m=0;m<b.length;m++){var n=b[m].x,t=b[m].y;if((n-k)*(n-k)+(t-g)*(t-g)<=4*f*f){e=!1;break}}return e?(b.push(d),"visible"):"hidden"};x.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});n.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",d);n.selectAll("g.inner").selectAll(".graph.binary").attr("visibility",d)}function G(d,g){phyd3.phylogram.lineAreas=
[];phyd3.phylogram.dx=0;g=d3.layout.cluster().separation(function(a,b){return 1}).children(a.children||function(a){return a.branchset}).size([R*a.scaleY,I*a.scaleX]);F=g(b);a.showPhylogram||phyd3.phylogram.scaleBranchLengths(F,I*a.scaleX);var f=n.selectAll("path.link");d||(f=f.data(g.links(F)),f.enter().append("svg:path").attr("class","link").attr("fill","none").attr("stroke",u()));f.attr("d",phyd3.phylogram.rightAngleDiagonal);S=void 0;c=0;f=F.filter(function(a){return!a.children});for(g=0;g<f.length;g++)f[g].show=
a.dynamicHide?qa(f[g],!a.lineupNodes):!0;S=void 0;f=F.filter(function(a){return a.children});for(g=0;g<f.length;g++)f[g].show=a.dynamicHide?qa(f[g],!0):!0;d||(N=N.data(F),N.enter().append("svg:g").attr("class",function(a){return a.children?0==a.depth?"root node cid_"+a.id:"inner node cid_"+a.id:"leaf node cid_"+a.id}).style("cursor","pointer").on("click",function(d){var f=d3.event;f.ctrlKey?(ra(b,d.id),G(!0)):f.altKey?(sa(b,d.id),e()):void 0!==a.popupAction?a.popupAction(d):ua(d)}).style("z-index",
100).append("rect").attr("class","pointer").attr("fill","red").attr("width",2*a.nodeHeight+2+"px").attr("height",2*a.nodeHeight+2+"px").style("opacity","0").attr("x","-"+(a.nodeHeight+1)+"px").attr("y","-"+(a.nodeHeight+1)+"px"));N.attr("transform",function(a){return"translate("+parseInt(a.y)+","+parseInt(a.x)+")"});phyd3.phylogram.lineAreas=[]}function ma(b,d){if(b.name&&b.name.match(d)){var e=!b.children&&a.lineupNodes?phyd3.phylogram.dx-b.y:0;n.selectAll("g.node.cid_"+b.id).append("rect").attr("class",
