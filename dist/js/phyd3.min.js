...
"legend").text(0!=p.legend.show?p.legend.fields[l].name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+l*(2*e+5)+2*e)+", -10) rotate(-90)");w+=(p.legend.fields.length+1)*(2*e+5)+5;break;case "multibar":for(l=0;l<p.legend.fields.length;l++)D[p.id][l]=D[p.id][l].range([0,a.graphWidth]);g.selectAll("rect.multibar.gid"+p.id).attr("height",p.legend.stacked?parseInt(2*e/p.legend.fields.length):parseInt(2*e)).attr("width",function(a,b){b=parseInt(D[p.id]&&D[p.id][a.i]?D[p.id][a.i](a.value):0);
a=parseInt(D[p.id]&&D[p.id][a.i]?D[p.id][a.i](0):0);return b>a?b-a:a-b}).attr("transform",function(b,d){d=w;var g=-1*e;p.legend.stacked?g+=2*e/p.legend.fields.length*b.i:d+=b.i*(a.graphWidth+5);var f=parseInt(D[p.id]&&D[p.id][b.i]?D[p.id][b.i](b.value):0);b=parseInt(D[p.id]&&D[p.id][b.i]?D[p.id][b.i](0):0);return" translate("+parseInt(d+(b<f?b:f))+", "+parseInt(g)+")"}).attr("x",function(a,b){});if(a.showGraphs&&a.showGraphLegend)if(p.legend.stacked)n.append("text").attr("class","legend").text(0!=
p.legend.show?p.name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+a.graphWidth/2)+",-10) rotate(-90)");else for(l=0;l<p.legend.fields.length;l++)n.append("text").attr("class","legend").text(0!=p.legend.show?p.legend.fields[l].name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+l*(a.graphWidth+5)+a.graphWidth/2)+",-10) rotate(-90)");w+=(p.legend.stacked?1:p.legend.fields.length)*(a.graphWidth+5);break;case "boxplot":E[p.id]=E[p.id].range([0,a.graphWidth]);g.selectAll("rect.q1.boxplot.gid"+
p.id).attr("height",parseInt(2*e)).attr("width",function(a){return parseInt(E[p.id](a.median)-E[p.id](a.q1))}).attr("transform",function(a,b){a=w+E[p.id](a.q1);return" translate("+parseInt(a)+",-"+parseInt(e)+")"});g.selectAll("rect.q3.boxplot.gid"+p.id).attr("height",parseInt(2*e)).attr("width",function(a){return parseInt(E[p.id](a.q3)-E[p.id](a.median))}).attr("transform",function(a,b){a=w+E[p.id](a.median);return" translate("+parseInt(a)+",-"+parseInt(e)+")"});g.selectAll("line.baseline.boxplot.gid"+
p.id).attr("y1",parseInt(e)).attr("y2",parseInt(e)).attr("x1",function(a){return E[p.id](a.min)}).attr("x2",function(a){return E[p.id](a.max)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(e)+")"});g.selectAll("line.median.boxplot.gid"+p.id).attr("y1",0).attr("y2",parseInt(2*e)).attr("x1",function(a){return E[p.id](a.median)}).attr("x2",function(a){return E[p.id](a.median)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(e)+")"});a.showGraphs&&
a.showGraphLegend&&n.append("text").attr("class","legend").text(0!=p.legend.show?p.name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+2*e)+", -10) rotate(-90)");w+=a.graphWidth+5;break;case "heatmap":g.selectAll("rect.heatmap.gid"+p.id).attr("width",2*e).attr("height",2*e).attr("transform",function(a,b){return" translate("+(w+2*a.i*e)+",-"+e+")"});if(a.showGraphs&&a.showGraphLegend)for(l=0;l<p.legend.fields.length;l++)n.append("text").attr("class","legend").text(0!=p.legend.show?p.legend.fields[l].name:
"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+d+w+(l+1)*e*2)+",-10) rotate(-90)");w+=2*p.legend.fields.length*e+5}}n.selectAll("text.legend").attr("fill",A()).attr("font-size",2*a.nodeHeight+"px")}}function Ca(){var e=a.nodeHeight;w=0;if(b.graphs)for(var d=0;d<b.graphs.length;d++){var g=b.graphs[d];switch(g.type){case "pie":w+=2*e+5;break;case "binary":w+=(g.legend.fields.length+1)*(2*e+5)+5;break;case "multibar":w+=g.legend.fields.length*(a.graphWidth+5);break;case "heatmap":w+=2*
g.legend.fields.length*e+5}}return 2*a.margin+(a.showNodeNames?Q:0)+(a.showGraphs?w:0)+(a.showDomains?a.domainWidth:0)}function xa(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";for(var e=!0,g=a.nodeHeight,f=d.x,m=d.y,h=0;h<b.length;h++){var q=b[h].x,n=b[h].y;if((q-f)*(q-f)+(n-m)*(n-m)<=4*g*g){e=!1;break}}return e?(b.push(d),"visible"):"hidden"};C.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});n.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",
