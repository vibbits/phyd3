...
m=u.value,g=l.value;e.value="translate("+a.marginX+", "+a.marginY+")";u.value=f.width+a.marginX;l.value=f.height+a.marginY;if("undefined"!=typeof window.XMLSerializer)var n=(new XMLSerializer).serializeToString(d);else"undefined"!=typeof d.xml&&(n=d.xml);e.value=h;u.value=m;l.value=g;return n}function p(){a.showDomains?(I=phyd3.phylogram.scaledomainWidths(D,a.domainWidth),w.append("svg:path").attr("class","domain").attr("stroke",e()).attr("visibility",function(a,d){return a.show?"visibile":"hidden"}),
E=E.data(function(a,d,f){a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.domains:[];for(f=0;f<a.length;f++)a[f].i=d;a.sort(function(a,f){return Math.abs(f.to-f.from)-Math.abs(a.to-a.from)});return a}).enter().append("svg:g").attr("class","domain"),E.append("svg:rect").attr("class","domain hover-visible").attr("stroke",a.foregroundColor).attr("stroke-width",a.outline+"px").append("title").text(function(a){return a.name}),v(),F(),x()):(E.remove(),w.selectAll("path.domain").remove(),
E=w.selectAll("g.domain"))}function k(){a.showDomainNames?(E.append("svg:text").attr("class","domain").attr("dy",-3).attr("fill",e()).attr("font-size","10px").text(function(a){return a.name}),F()):E.selectAll("text.domain").remove()}function v(){var d=w.data();w.selectAll("g.domain").attr("visibility",function(b,f,r){return d[r].show&&b.confidence<=a.domainLevel?"visible":"hidden"});w.selectAll("path.domain").attr("visibility",function(a){return a.show?"visible":"hidden"});d3.select("#domainLevel").attr("value",
a.domainLevel.toPrecision(1))}function x(){E.selectAll("rect.domain").attr("fill",function(b){return a.showDomainColors&&d.domcolors[b.name]?d.domcolors[b.name].color.replace(/0x/,"#"):e()})}function F(){var d=w.data(),b=K+(a.showLabels?L:0)+(a.showGraphs?t:0);I=phyd3.phylogram.scaledomainWidths(D,a.domainWidth);w.selectAll("path.domain").attr("d",function(f,e,r){e=a.lineupNodes?phyd3.phylogram.dx-d[r].y:0;f=f.sequences&&f.sequences[0]&&f.sequences[0].domainArchitecture?f.sequences[0].domainArchitecture.sequenceLength:
0;return"M"+parseInt(b+e)+",0L"+parseInt(b+e+I(f))+",0"});E.selectAll("rect.domain").attr("width",function(a){return parseInt(I(a.to-a.from))}).attr("transform",function(f,e,r){return"translate("+parseInt(b+(a.lineupNodes?phyd3.phylogram.dx-d[f.i].y:0)+I(f.from))+","+parseInt(-1*a.nodeHeight)+")"}).attr("height",2*a.nodeHeight-2);E.selectAll("text.domain").attr("transform",function(f){return"translate("+parseInt(b+(a.lineupNodes?phyd3.phylogram.dx-d[f.i].y:0)+I(f.from))+","+parseInt(-1*a.nodeHeight)+
")"});d3.select("#domainWidth").attr("value",a.domainWidth)}function ca(){a.showSupportValues?(n.selectAll("g.inner.node").append("svg:text").attr("class","supportValue").attr("dx",-2).attr("text-anchor","end").attr("fill",a.supportValuesColor).text(function(a){var d="";if(a.confidences)for(var f in a.confidences)d+=parseFloat(a.confidences[f].value)+" ";return d}),B()):n.selectAll("text.supportValue").remove()}function da(){a.showLengthValues?(n.selectAll("g.node").append("svg:text").attr("class",
"branchLength").attr("dx",-2).attr("text-anchor","end").attr("fill",a.branchLengthColor).text(function(a){return a.branchLength?parseFloat(a.branchLength).toFixed(5):""}),B()):n.selectAll("text.branchLength").remove()}function X(a,d,f){var b=a.attr("id")+"_"+a.selectAll("div.panel-default").size(),e=a.append("div").attr("class","panel panel-default");e.append("div").attr("class","panel-heading").attr("role","tab").attr("id","heading"+b).append("h4").attr("class","panel-title").append("a").attr("class",
"collapsed").attr("aria-expanded",f?"true":"false").attr("role","button").attr("data-toggle","collapse").attr("data-parent","#"+a.attr("id")).attr("href","#collapse"+b).attr("aria-controls","collapse"+b).html(d);return e.append("div").attr("id","collapse"+b).attr("class","panel-collapse collapse "+(f?"in":"")).attr("role","tabpanel").attr("aria-labelledby","heading"+b).append("div").attr("class","panel-body table-responsive")}function ma(e){var q=d3.event;if(!q.defaultPrevented){var f=q.layerX;y=
q.layerY;var r="popup"+parseInt(Date.now()*Math.random()*1E3);console.log(q);var f=d3.select(b).append("div").attr("class","popup").attr("id",r).style("position","absolute").style("top",parseInt(y)+"px").style("left",parseInt(f)+"px").style("width",parseInt(a.popupWidth)+"px").style("color",a.foregroundColor).style("background-color",a.backgroundColor),l=f.append("button").attr("class","btn btn-link").style("position","absolute").style("right","8px");l.append("span").attr("class","glyphicon glyphicon-remove").attr("aria-hidden",
"true");l.on("click",function(){d3.select("#"+r).remove();n.selectAll("g.node.cid_"+e.id).selectAll("rect.pointer").style("opacity","0")});f=f.append("div").attr("class","panel-group").attr("id","accordion_"+parseInt(Date.now()*Math.random()*1E3)).attr("role","tablist").attr("aria-multiselectable","true");l="text"==q.target.nodeName||q.target.classList.contains("pointer")?!0:!1;l=X(f,"Node",l).append("table").attr("class","table table-condensed table-bordered").append("tbody");if(e.name){var h=l.append("tr");
h.append("td").text("Name");h.append("td").text(e.name)}if(e.taxonomies)for(var m in e.taxonomies){var g=e.taxonomies[m],h=l.append("tr");h.append("td").text("Taxononomy");h=h.append("td");d.taxcolors[g.code]&&d.taxcolors[g.code].url&&(h=h.append("a").attr("href",d.taxcolors[g.code].url).attr("target","_blank"));var k="",k=k+(d.taxcolors[g.code]&&d.taxcolors[g.code].name?d.taxcolors[g.code].name+" ":""),k=k+" ",k=k+(g.scientificName?"\x3ci\x3e"+g.scientificName+"\x3c/i\x3e":""),k=k+" ",k=k+(g.commonName?
g.commonName:""),k=k+" ",k=k+(g.code?"("+g.code+")":"");h.html(k)}if(e.confidences)for(var p in e.confidences)h=l.append("tr"),h.append("td").text("Confidence"),h.append("td").html(e.confidences[p].value);h=l.append("tr");h.append("td").text("Depth");h.append("td").text(e.depth);h=l.append("tr");h.append("td").text("Branch length");h.append("td").text(e.branchLength?e.branchLength:0);h=l.append("tr");h.append("td").text("Distance from root");h.append("td").text(e.rootDist);for(var C in e.properties)h=
e.properties[C],z=h.ref,m=h.value,m=m.trim(),/^http:\/\//.test(m)&&(m="\x3ca href\x3d'"+m+"' target\x3d'_blank'\x3e"+m+"\x3c/a\x3e\x3cbr /\x3e"),h=l.append("tr"),h.append("td").text(z),h.append("td").html(m);for(var z in e.events)m=e.events[z],h=l.append("tr"),h.append("td").text(z),h.append("td").html(m);if(e.sequences)for(var t in e.sequences)if(e.sequences[t].domainArchitecture&&e.sequences[t].domainArchitecture.domains){l=q.target.parentNode.classList.contains("domain")?!0:!1;l=X(f,"Domains",
l).append("table").attr("id","dt"+r).attr("class","table table-condensed table-bordered");z=l.append("thead").append("tr");z.append("th").text("#");z.append("th").text("Name");z.append("th").text("Description");z.append("th").text("From");z.append("th").text("To");z.append("th").text("P value");l=l.append("tbody");for(C=0;C<e.sequences[t].domainArchitecture.domains.length;C++)m=e.sequences[t].domainArchitecture.domains[C],h=l.append("tr"),h.append("td").style("background",d.domcolors[m.name]&&d.domcolors[m.name].color?
d.domcolors[m.name].color.replace("0x","#"):"").text(" "),z=h.append("td"),d.domcolors[m.name]&&d.domcolors[m.name].url&&(z=z.append("a").attr("href",d.domcolors[m.name].url).attr("target","_blank")),z.text(m.name),h.append("td").text(d.domcolors[m.name]?d.domcolors[m.name].description:""),h.append("td").text(m.from),h.append("td").text(m.to),h.append("td").text(m.confidence);$("#dt"+r).dataTable({searching:!1,paging:!1,language:{info:""},order:[[3,"asc"]],columns:[{orderable:!1},null,null,null,null,
null]})}if(d.graphs)for(t=0;t<d.graphs.length;t++)if(m=d.graphs[t],m.data&&m.data[e.id]&&m.data[e.id].length)for(l=q.target.parentNode.classList.contains(m.type)?!0:!1,l=X(f,m.name,l).append("table").attr("class","table table-condensed table-bordered").append("tbody"),C=0;C<m.legend.fields.length;C++)h=l.append("tr"),z=h.append("td"),m.legend.fields[C].url&&(z=z.append("a").attr("href",m.legend.fields[C].url).attr("target","_blank")),z.text(m.legend.fields[C].name),h.append("td").text(m.data[e.id][C]);
n.selectAll("g.node.cid_"+e.id).selectAll("rect.pointer").style("opacity","1");q.stopPropagation()}}function ea(){a.lineupNodes?n.selectAll("g.leaf").append("path").attr("class","support").attr("stroke",e()).attr("stroke-dasharray","2,3").attr("stroke-width","0.5px"):n.selectAll("path.support").remove()}function S(b){if(b.children&&"only leaf"==a.showNodesType||!b.children&&"only inner"==a.showNodesType)return"";var e="";if(a.showTaxonomy&&b.taxonomies)for(var f in b.taxonomies){var r=b.taxonomies[f];
a.showFullTaxonomy&&(e+=d.taxcolors[r.code]&&d.taxcolors[r.code].name?d.taxcolors[r.code].name:"",e+=" ",e+=r.scientificName?r.scientificName:"",e+=" ",e+=r.commonName?r.commonName:"",e+=" ");e+=r.code?r.code:"";e+=" "}e+=(a.showNodeNames&&b.name?b.name:"")+" ";if(a.showSequences&&b.sequences)for(var l in b.sequences)if(f=b.sequences[l],e+=f.symbol?f.symbol:"",e+=" ",e+=f.accession?f.accession.value:"",e+=" ",e+=f.name?f.name:"",e+=" ",e+=f.geneName?f.geneName:"",e+=" ",f.annotations)for(var h in f.annotations)e+=
f.annotations[h].desc?f.annotations[h].desc:"",e+=" ";return e}function T(){var d=0;w.selectAll("text.name").text(function(a){a=S(a);a.length>d&&(d=a.length);return a});n.selectAll("g.node.inner").selectAll("text.name").text(S);a.showGraphs&&M();a.showDomains&&F()}function J(){w.selectAll("text.name").attr("visibility",function(a){return a.show?"visible":"hidden"});a.lineupNodes&&w.selectAll("path.support").attr("visibility",function(a){return a.show?"visible":"hidden"});a.showLabels&&fa();a.showGraphs&&
ga();a.showDomains&&v()}function Y(){n.selectAll("text.name").attr("stroke",e()).attr("stroke-width",a.outline+"px").attr("fill",function(b){var r=null;if(a.showTaxonomyColors&&d.taxcolors&&b.taxonomies)for(var f in b.taxonomies){var g=b.taxonomies[f];d.taxcolors[g.code]&&d.taxcolors[g.code].color&&(r=d.taxcolors[g.code].color.replace(/0x/,"#"))}return r?r:e()})}function ha(){w.selectAll("text.name").attr("dx",function(d){return parseInt(a.lineupNodes?phyd3.phylogram.dx-d.y+5:5)});w.selectAll("path.support").attr("d",
function(a){return"M0,0 L"+parseInt(phyd3.phylogram.dx-a.y)+",0"});a.showLabels&&Z();a.showGraphs&&M();a.showDomains&&F()}function B(){n.selectAll("text.name").attr("dy",a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");n.selectAll("text.branchLength").attr("dy",-a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");n.selectAll("text.supportValue").attr("dy",a.nodeHeight/2+5).attr("font-size",1.5*a.nodeHeight+"px");K=100*a.nodeHeight/6+10;d3.select("#nodeHeight").attr("value",a.nodeHeight);
ha();a.showLabels&&Z();a.showGraphs&&M();a.showDomains&&F()}function ia(){if(a.showLabels&&d.labels){console.log(d.labels);for(var b=0;b<d.labels.length;b++){var q=d.labels[b];q.id||(q.id=parseInt(Date.now()*Math.random()*1E3));q.data.tag&&n.selectAll("g.node").each(function(a){var d=a[q.data.tag],b=q.data.ref,b=b?b:"value",d="object"==typeof d?d[b]:d;if("property"==q.data.tag)for(pid in a.properties){var e=a.properties[pid];if(e.ref==b){d=e.value;break}}d&&(q.data[a.id]=d)});switch(q.type){case "text":for(cid in q.data)q.data[cid]&&
n.selectAll(".cid_"+cid).append("text").attr("class","nodelabel text lid"+q.id).attr("dy",3).attr("text-anchor","start").attr("stroke-width",a.outline+"px").attr("stroke",e()).attr("fill",e()).attr("font-size",1.5*a.nodeHeight+"px").attr("visibility","visible").text(q.data[cid]).append("title").text(q.name+": "+q.data[cid]);break;case "color":for(cid in q.data)if(q.data[cid]){var f=a.nodeHeight,g=q.data[cid].replace(/0x/,"#");n.selectAll(".cid_"+cid).append("rect").attr("height",2*f).attr("width",
2*f).attr("class","nodelabel color lid"+q.id).attr("style","fill: "+g+"; stroke: "+g).append("title").text(q.name)}}}fa();Z()}else n.selectAll(".nodelabel").remove(),L=0;M();F()}function Z(){var b=a.nodeHeight;L=0;if(d.labels){for(var e=0;e<d.labels.length;e++){var f=d.labels[e],g=0;w.selectAll(".nodelabel.lid"+f.id).attr("transform",function(d){var e=0,m=0;"color"==f.type&&(e=m=-b);var r=0;"text"==this.nodeName&&this.firstChild?r=this.firstChild.length*b/1.5:"rect"==this.nodeName&&(r=2*b);r>g&&(g=
...
b,f.legend.fields[1]={name:f.legend.fields[0].name,color:f.legend.fields[0].background?f.legend.fields[0].background:phyd3.phylogram.randomColor(),invert:!0}))});switch(f.type){case "pie":for(cid in f.data)if(f.data[cid]&&Array.isArray(f.data[cid])){var l=!1;n.select(".cid_"+cid).each(function(a){0==a.branchset.length&&(l=!0)});for(var u=n.selectAll(".cid_"+cid).append("svg:g").attr("class","graph pie gid"+f.id),h=[],m=0;m<f.data[cid].length;m++)void 0!=f.data[cid][m]&&h.push({i:m,value:f.data[cid][m],
leaf:l});u=u.selectAll("path.pie.gid"+f.id).data(b(h)).enter().append("path").attr("class","pie hover-visible gid"+f.id).attr("stroke",e()).attr("stroke-width",a.outline+"px").attr("fill",function(a,b){return f.legend.fields?(f.legend.fields[b]||(f.legend.fields[b]={},f.legend.fields[b].name="Series "+(b+1),f.legend.fields[b].color=phyd3.phylogram.randomColor()),f.legend.fields[b].color.replace(/0x/,"#")):""}).append("title").text(function(a,b){return f.legend.fields?(f.legend.fields[b]&&f.legend.fields[b].name?
f.legend.fields[b].name+": ":"")+(f.legend.fields[b].invert?f.data.max-a.data.value:a.data.value):""})}break;case "binary":for(cid in f.data)if(f.data[cid]&&Array.isArray(f.data[cid])){h=[];l=!1;n.select(".cid_"+cid).each(function(a){0==a.branchset.length&&(l=!0)});u=n.selectAll(".cid_"+cid).append("svg:g").attr("class","graph binary gid"+f.id);for(m=0;m<f.data[cid].length;m++)void 0!=f.data[cid][m]&&h.push({i:m,value:f.data[cid][m],shape:f.legend.fields[m]?f.legend.fields[m].shape:"",leaf:l});u=
u.selectAll("path.binary.gid"+f.id).data(h).enter().append("path").attr("class","binary hover-visible gid"+f.id).attr("style",function(a,b){b=(f.legend.fields[b]?f.legend.fields[b].color:"").replace(/0x/,"#");return"fill:"+(1<=a.value?b:"none")+";stroke:"+(0<=a.value?b:"none")}).append("title").text(function(a,b){return(f.legend.fields[b]?f.legend.fields[b].name+": ":"")+a.value})}break;case "multibar":N[f.id]=[];for(m=0;m<f.legend.fields.length;m++)u=d3.max(d3.values(f.data),function(a){if(a)return a[m]}),
N[f.id][m]=d3.scale.linear().domain([0,u]);for(cid in f.data)if(f.data[cid]){u=n.select(".cid_"+cid).append("svg:g").attr("class","graph multibar gid"+f.id);h=[];for(m=0;m<f.data[cid].length;m++)void 0!=f.data[cid][m]&&h.push({i:m,value:f.data[cid][m]});u=u.selectAll("rect.multibar.gid"+f.id).data(h).enter().append("rect").attr("class","multibar hover-visible gid"+f.id).attr("fill",function(a,b){return(f.legend.fields[b]?f.legend.fields[b].color:"").replace(/0x/,"#")}).attr("stroke",a.foregroundColor).attr("stroke-width",
a.outline).append("title").text(function(a,b){return(f.legend.fields[b]?f.legend.fields[b].name+": ":"")+a.value})}break;case "boxplot":A[f.id]=null;u=0;h=Number.MAX_SAFE_INTEGER;console.log(h);for(m=0;5>m;m++){var k=d3.max(d3.values(f.data),function(a){if(a)return a[m]}),p=d3.max(d3.values(f.data),function(a){if(a)return a[m]});k>u&&(u=k);p<h&&(h=p)}A[f.id]=d3.scale.linear().domain([h,u]);console.log(f.data);for(cid in f.data)f.data[cid]&&(u=n.select(".cid_"+cid).append("svg:g").attr("class","graph boxplot gid"+
f.id),h=[],h.push({min:f.data[cid][0],q1:f.data[cid][1],median:f.data[cid][2],q3:f.data[cid][3],max:f.data[cid][4]}),u=u.selectAll("rect.boxplot.gid"+f.id).data(h).enter(),u.append("line").attr("class","boxplot baseline hover-visible gid"+f.id).attr("style","marker-start: url(#markerBoxplot); marker-end: url(#markerBoxplot);").attr("stroke",e()),u.append("rect").attr("class","boxplot q1 hover-visible gid"+f.id).attr("fill",(f.legend.fields[0]?f.legend.fields[0].color:"").replace(/0x/,"#")).attr("stroke",
a.foregroundColor).attr("stroke-width",a.outline).append("title").text(function(a){return"Min: "+a.min+", Q1: "+a.q1+", Median: "+a.median}),u.append("rect").attr("class","boxplot q3 hover-visible gid"+f.id).attr("fill",(f.legend.fields[1]?f.legend.fields[1].color:"").replace(/0x/,"#")).attr("stroke",e()).attr("stroke-width",a.outline).append("title").text(function(a){return"Median: "+a.median+", Q3: "+a.q3+", Max: "+a.max}),u.append("line").attr("class","boxplot median hover-visible gid"+f.id).attr("stroke",
e()).attr("stroke-width","2px"));break;case "heatmap":var k=d3.max(d3.values(f.data),function(a){if(a)return d3.max(a)}),p=d3.min(d3.values(f.data),function(a){if(a)return d3.min(a)}),v=d3.scale.quantize().domain([p,k]).range(colorbrewer[f.legend.gradient.name][f.legend.gradient.classes]);for(cid in f.data)if(f.data[cid]){u=n.select(".cid_"+cid).append("svg:g").attr("class","graph heatmap gid"+f.id);h=[];if(f.data[cid])for(m=0;m<f.data[cid].length;m++)void 0!=f.data[cid][m]&&h.push({i:m,value:f.data[cid][m]});
u=u.selectAll("rect.heatmap.gid"+f.id).data(h).enter().append("rect").attr("class","heatmap hover-visible gid"+f.id).attr("fill",function(a,b){return v(a.value)}).attr("stroke",a.foregroundColor).attr("stroke-width",a.outline+"px").append("title").text(function(a,b){return(f.legend.fields[b]?f.legend.fields[b].name+": ":"")+a.value})}break;default:console.log("Graph not supported "+f.type)}}ga();M()}else t=0,n.selectAll("g.graph").remove(),n.selectAll("text.legend").remove();F()}function M(){var b=
a.nodeHeight,g=K+L;w.selectAll("g.graph").attr("transform",function(b){return"translate("+(a.lineupNodes?phyd3.phylogram.dx-b.y+g:g)+",0)"});var f=n.selectAll("g.graph");d3.select("#graphWidth").attr("value",a.graphWidth);n.selectAll("text.legend").remove();t=0;if(d.graphs){for(var k=0;k<d.graphs.length;k++){var l=d.graphs[k];switch(l.type){case "pie":f.selectAll("path.pie.gid"+l.id).attr("d",function(a){if(0<a.value)return d3.svg.arc().innerRadius(0).outerRadius(b)(a)}).attr("transform",function(a){return" translate("+
parseInt(a.data.leaf?b+t:0)+",0)"});a.showGraphLegend&&n.append("text").attr("class","legend").text(0!=l.legend.show?l.name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+g+t+2*b)+",-10) rotate(-90)");t+=2*b+5;break;case "binary":f.selectAll("path.binary.gid"+l.id).attr("d",function(a){var d=d3.svg.symbol().size(4*(b-2)*(b-2));d.type(l.legend.fields[a.i].shape);return d(a)}).attr("transform",function(a){return" translate("+(a.leaf?t+10+a.i*(2*b+5):0)+" ,0)"});if(a.showGraphLegend)for(var h=
0;h<l.legend.fields.length;h++)n.append("text").attr("class","legend").text(0!=l.legend.show?l.legend.fields[h].name:"").attr("transform","translate("+(phyd3.phylogram.dx+g+t+h*(2*b+5)+2*b)+", -10) rotate(-90)");t+=(l.legend.fields.length+1)*(2*b+5)+5;break;case "multibar":for(h=0;h<l.legend.fields.length;h++)N[l.id][h]=N[l.id][h].range([0,a.graphWidth]);f.selectAll("rect.multibar.gid"+l.id).attr("height",parseInt(2*b)).attr("width",function(a,b){return parseInt(N[l.id][a.i]?N[l.id][a.i](a.value):
0)}).attr("transform",function(d,e){return" translate("+parseInt(t+d.i*(a.graphWidth+5))+",-"+parseInt(b)+")"});if(a.showGraphLegend)for(h=0;h<l.legend.fields.length;h++)n.append("text").attr("class","legend").text(0!=l.legend.show?l.legend.fields[h].name:"").attr("transform","translate("+(phyd3.phylogram.dx+g+t+h*(a.graphWidth+5)+a.graphWidth/2)+",-10) rotate(-90)");t+=l.legend.fields.length*(a.graphWidth+5);break;case "boxplot":A[l.id]=A[l.id].range([0,a.graphWidth]);f.selectAll("rect.q1.boxplot.gid"+
l.id).attr("height",parseInt(2*b)).attr("width",function(a){return parseInt(A[l.id](a.median)-A[l.id](a.q1))}).attr("transform",function(a,d){a=t+A[l.id](a.q1);return" translate("+parseInt(a)+",-"+parseInt(b)+")"});f.selectAll("rect.q3.boxplot.gid"+l.id).attr("height",parseInt(2*b)).attr("width",function(a){return parseInt(A[l.id](a.q3)-A[l.id](a.median))}).attr("transform",function(a,d){a=t+A[l.id](a.median);return" translate("+parseInt(a)+",-"+parseInt(b)+")"});f.selectAll("line.baseline.boxplot.gid"+
l.id).attr("y1",parseInt(b)).attr("y2",parseInt(b)).attr("x1",function(a){return A[l.id](a.min)}).attr("x2",function(a){return A[l.id](a.max)}).attr("transform",function(a,d){return" translate("+parseInt(t)+",-"+parseInt(b)+")"});f.selectAll("line.median.boxplot.gid"+l.id).attr("y1",0).attr("y2",parseInt(2*b)).attr("x1",function(a){return A[l.id](a.median)}).attr("x2",function(a){return A[l.id](a.median)}).attr("transform",function(a,d){return" translate("+parseInt(t)+",-"+parseInt(b)+")"});t+=a.graphWidth+
5;break;case "heatmap":f.selectAll("rect.heatmap.gid"+l.id).attr("width",2*b).attr("height",2*b).attr("transform",function(a,d){return" translate("+(t+2*a.i*b)+",-"+b+")"});if(a.showGraphLegend)for(h=0;h<l.legend.fields.length;h++)n.append("text").attr("class","legend").text(0!=l.legend.show?l.legend.fields[h].name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+g+t+(h+1)*b*2)+",-10) rotate(-90)");t+=2*l.legend.fields.length*b+5}}n.selectAll("text.legend").attr("fill",e()).attr("font-size",
2*a.nodeHeight)}}function na(){var b=a.nodeHeight;t=0;if(d.graphs)for(var e=0;e<d.graphs.length;e++){var f=d.graphs[e];switch(f.type){case "pie":t+=2*b+5;break;case "binary":t+=(f.legend.fields.length+1)*(2*b+5)+5;break;case "multibar":t+=f.legend.fields.length*(a.graphWidth+5);break;case "heatmap":t+=2*f.legend.fields.length*b+5}}return 2*a.margin+(a.showNodeNames?K:0)+(a.showGraphs?t:0)+(a.showDomains?a.domainWidth:0)}function ga(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";for(var e=
!0,f=a.nodeHeight,g=d.x,m=d.y,k=0;k<b.length;k++){var q=b[k].x,r=b[k].y;if((q-g)*(q-g)+(r-m)*(r-m)<=4*f*f){e=!1;break}}return e?(b.push(d),"visible"):"hidden"};w.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});n.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",d);n.selectAll("g.inner").selectAll(".graph.binary").attr("visibility",d)}function G(b){phyd3.phylogram.lineAreas=[];phyd3.phylogram.dx=0;var g=d3.layout.cluster().separation(function(a,b){return 1}).children(a.children||
function(a){return a.branchset}).size([O*a.scaleY,H*a.scaleX]);D=g(d);a.showPhylogram||phyd3.phylogram.scaleBranchLengths(D,H*a.scaleX);var f=n.selectAll("path.link");b||(f=f.data(g.links(D)),f.enter().append("svg:path").attr("class","link").attr("fill","none").attr("stroke",e()));f.attr("d",phyd3.phylogram.rightAngleDiagonal);P=void 0;c=0;f=D.filter(function(a){return!a.children});for(g=0;g<f.length;g++)f[g].show=a.dynamicHide?ka(f[g],!a.lineupNodes):!0;P=void 0;f=D.filter(function(a){return a.children});
for(g=0;g<f.length;g++)0<S(f[g]).trim().length?f[g].show=a.dynamicHide?ka(f[g],!0):!0:f[g].show=!1;b||(U=U.data(D),U.enter().append("svg:g").attr("class",function(a){return a.children?0==a.depth?"root node cid_"+a.id:"inner node cid_"+a.id:"leaf node cid_"+a.id}).style("cursor","pointer").on("click",void 0===a.popupAction?ma:a.popupAction).append("rect").attr("class","pointer").attr("fill","red").attr("width",2*a.nodeHeight+2+"px").attr("height",2*a.nodeHeight+2+"px").style("opacity","0").attr("x",
"-"+(a.nodeHeight+1)+"px").attr("y","-"+(a.nodeHeight+1)+"px"));U.attr("transform",function(a){return"translate("+parseInt(a.y)+","+parseInt(a.x)+")"});phyd3.phylogram.lineAreas=[]}function Q(){G(!0);V()}function ka(b,d){var e=!0,g=b.y+1,l=b.y+5.5*S(b).length,h=b.x-a.nodeHeight,m=b.x+a.nodeHeight;if(!P)return P=b,!0;a.lineupNodes&&!b.children&&(g+=phyd3.phylogram.dx-b.y,l+=phyd3.phylogram.dx-b.y);var k=P,r=k.y+1,q=k.y+5.5*S(k).length,n=k.x-a.nodeHeight,p=k.x+a.nodeHeight;a.lineupNodes&&!k.children&&
(r+=phyd3.phylogram.dx-k.y,q+=phyd3.phylogram.dx-k.y);if(r<=g&&g<=q&&n<=h&&h<=p||r<=l&&l<=q&&n<=h&&h<=p||r<=g&&g<=q&&n<=m&&m<=p||r<=l&&l<=q&&n<=m&&m<=p)e=!1;if(e&&d)for(d=0;d<phyd3.phylogram.lineAreas.length;d++)if(n=phyd3.phylogram.lineAreas[d],k=n.start[0],r=n.end[0],q=n.start[1],n=n.end[1],q==n){if(h<=q&&n<=m&&!(k>l||r<g)){e=!1;break}}else if(k==r&&g<=k&&r<=l&&!(q>m||n<h)){e=!1;break}e&&(P=b);return e}function V(){var b=aa.translate(),d=b[0]+a.marginX-a.translateX,e=b[1]+a.marginY-a.translateY;
n.attr("transform",function(a){return"translate("+parseInt(d)+","+parseInt(e)+") scale(1)"});0!=a.lastScale&&(ha(),a.dynamicHide&&J())}a=a||{};a.scaleY=a.scaleY||1;a.scaleX=a.scaleX||1;a.translateX=a.translateX||0;a.translateY=a.translateY||0;a.height=a.height||800;a.margin=a.margin||20;a.scaleStep=a.scaleStep||.3;a.nodeHeight=a.nodeHeight||6;a.nodeHeightStep=a.nodeHeightStep||1;a.textLength=a.textLength||100;a.domainWidth=a.domainWidth||100;a.domainWidthStep=a.domainWidthStep||100;a.graphWidth=a.graphWidth||
20;a.graphWidthStep=a.graphWidthStep||10;a.domainLevel=a.domainLevel||1;a.domainLevelStep=a.domainLevelStep||10;a.outline=a.outline||.3;a.popupWidth=a.popupWidth||500;a.foregroundColor=a.foregroundColor||"#000";a.backgroundColor=a.backgroundColor||"#fff";a.branchLengthColor=a.branchLengthColor||"red";a.supportValuesColor=a.supportValuesColor||"blue";a.showNodesType=a.showNodesType||"only leaf";a.treeWidth=a.treeWidth||"auto";a.showFullTaxonomy="showFullTaxonomy"in a?a.showFullTaxonomy:!1;a.showLabels=
"showLabels"in a?a.showLabels:!0;a.showDomains="showDomains"in a?a.showDomains:!0;a.dynamicHide="dynamicHide"in a?a.dynamicHide:!1;a.invertColors="invertColors"in a?a.invertColors:!1;a.lineupNodes="lineupNodes"in a?a.lineupNodes:!0;a.showSupportValues="showSupportValues"in a?a.showSupportValues:!1;a.showLengthValues="showLengthValues"in a?a.showLengthValues:!1;a.showTaxonomy="showTaxonomy"in a?a.showTaxonomy:!0;a.showTaxonomyColors="showTaxonomyColors"in a?a.showTaxonomyColors:!0;a.showDomainNames=
"showDomainNames"in a?a.showDomainNames:!1;a.showDomainColors="showDomainColors"in a?a.showDomainColors:!0;a.showGraphs="showGraps"in a?a.showGraphs:!0;a.showGraphLegend="showGraphLegend"in a?a.showGraphLegend:!0;a.showNodeNames="showNodeNames"in a?a.showNodeNames:!0;a.showPhylogram="showPhylogram"in a?a.showPhylogram:!1;var D,I,P,A=[],N=[],L=100,K=100,t=0,R=100,la=!1;if(d.graphs)for(var ba=0;ba<d.graphs.length;ba++)if(0!=d.graphs[ba].legend.show){la=!0;break}R=la?R:0;a.marginX=a.margin;a.marginY=
a.margin+(a.showGraphs?R:0);var W=$(b).width(),H="auto"==a.treeWidth?W-na():a.treeWidth,O=a.height-(2*a.margin+(a.showGraphs?R:0)),H=parseInt(H),O=parseInt(O);0>H&&(H=W-2*K-(a.showGraphs?2*R:0));var aa=d3.behavior.zoom().scaleExtent([1,1]).on("zoom",function(b){b=0;var d=d3.event.sourceEvent;if(d){if(0<d.deltaY||0>d.wheelDelta)b=-1*a.scaleStep;if(0>d.deltaY||0<d.wheelDelta)b=a.scaleStep;var e=.5,g=.5;d.altKey||d.ctrlKey||(e=d.layerX/W,g=d.layerY/a.height);if(d.ctrlKey||!d.altKey)a.scaleY+=b,a.scaleY<
a.scaleStep?a.scaleY=a.scaleStep:a.translateY+=O*b*g;if(d.altKey||!d.ctrlKey)a.graphWidth+=0==b?0:0>b?-10:10,10>a.graphWidth&&(a.graphWidth=10),a.domainWidth+=0==b?0:0>b?-1*a.domainWidthStep:a.domainWidthStep,a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep),a.scaleX+=b,a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX+=H*b*e;a.lastScale=b;0!=a.lastScale?requestAnimFrame(Q):requestAnimFrame(V);d.preventDefault()}});d3.select(b).select("svg").remove();var n=d3.select(b).insert("svg").attr("width",
W+"px").attr("height",a.height+"px").attr("overflow","hidden").attr("version","1.1").attr("font-family","Open Sans").attr("xmlns","http://www.w3.org/2000/svg").call(aa);n.append("svg:rect").attr("class","canvas").attr("width","100%").attr("height","100%").attr("fill",a.invertColors?a.foregroundColor:a.backgroundColor);n.append("svg:text").attr("dy","20px").attr("stroke",e()).attr("stroke-width","1px").attr("font-size","15px").text(d.name?d.name:"");n.append("svg:text").attr("dy","40px").attr("stroke",
e()).attr("stroke-width","0.5px").attr("font-size","10px").text(d.description?d.description:"");n.append("defs").append("marker").attr("id","markerBoxplot").attr("markerWidth","1").attr("markerHeight","10").attr("refX","1").attr("refY","5").append("line").attr("x1","0").attr("x2","0").attr("y1","0").attr("y2","10").attr("stroke",e()).attr("stroke-width","2px");var n=n.append("svg:g").attr("id","main").attr("transform","translate("+a.marginX+", "+a.marginY+")"),U=n.selectAll("g.node");G();0==phyd3.phylogram.dx&&
(d3.select("#phylogram").attr("checked","").attr("disabled","disabled"),a.showPhylogram=!0,G());var w=n.selectAll("g.leaf.node"),E=w.selectAll("g.domain");ca();da();p();k();ja();ea();ia();U.append("svg:text").attr("class","name").attr("dx",a.nodeHeight+1).attr("dy",3).attr("text-anchor","start");Y();T();J();V();B();d3.select("#dynamicHide").on("click",function(){a.dynamicHide=!a.dynamicHide;G(!0);J()});d3.select("#invertColors").on("click",function(){a.invertColors=!a.invertColors;d3.selectAll(".canvas").attr("fill",
a.invertColors?a.foregroundColor:a.backgroundColor);var b=e();n.selectAll("path.link").attr("stroke",b);n.selectAll("text.legend").attr("fill",b);w.selectAll("path.support").attr("stroke",b);w.selectAll("path.domain").attr("stroke",b);w.selectAll("text.nodelabel").attr("stroke",b).attr("fill",b);Y();x()});d3.select("#phylogram").on("click",function(){a.showPhylogram=!a.showPhylogram;G(!0);B();J()});d3.select("#lineupNodes").on("click",function(){a.lineupNodes=!a.lineupNodes;G(!0);ea();B();J()});d3.select("#lengthValues").on("click",
function(){a.showLengthValues=!a.showLengthValues;da()});d3.select("#supportValues").on("click",function(){a.showSupportValues=!a.showSupportValues;ca();J()});d3.select("#nodeNames").on("click",function(){a.showNodeNames=!a.showNodeNames;T();B()});d3.select("#nodeLabels").on("click",function(){a.showLabels=!a.showLabels;ia()});d3.select("#taxonomy").on("click",function(){a.showTaxonomy=!a.showTaxonomy;T();B()});d3.select("#sequences").on("click",function(){a.showSequences=!a.showSequences;T();B()});
d3.select("#nodesType").on("change",function(){a.showNodesType=$(this).val();Q();T()});d3.select("#taxonomyColors").on("click",function(){a.showTaxonomyColors=!a.showTaxonomyColors;Y()});d3.select("#nodeHeightLower").on("click",function(){a.nodeHeight-=a.nodeHeightStep;a.nodeHeight<a.nodeHeightStep&&(a.nodeHeight=a.nodeHeightStep);G(!0);B();J()});d3.select("#nodeHeightHigher").on("click",function(){a.nodeHeight+=a.nodeHeightStep;G(!0);B();J()});d3.select("#resetZoom").on("click",function(){a.domainWidth=
a.domainWidthStep;a.scaleX=1;a.scaleY=1;a.translateX=0;a.translateY=0;G(!0);aa.translate([0,0]);V();B()});d3.select("#zoominY").on("click",function(){a.scaleY+=a.scaleStep;a.translateY+=O*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(Q)});d3.select("#zoomoutY").on("click",function(){a.scaleY-=a.scaleStep;a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY-=O*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(Q)});d3.select("#zoominX").on("click",function(){a.scaleX+=a.scaleStep;
a.translateX+=H*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(Q)});d3.select("#zoomoutX").on("click",function(){a.scaleX-=a.scaleStep;a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX-=H*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(Q)});d3.select("#domains").on("click",function(){a.showDomains=!a.showDomains;a.showDomains?(d3.select("#domainNames").attr("disabled",null),d3.select("#domainColors").attr("disabled",null),d3.select("#domainWidthLower").attr("disabled",null),
d3.select("#domainWidthHigher").attr("disabled",null),d3.select("#domainLevelLower").attr("disabled",null),d3.select("#domainLevelHigher").attr("disabled",null)):(d3.select("#domainNames").attr("disabled","disabled"),d3.select("#domainColors").attr("disabled","disabled"),d3.select("#domainWidthLower").attr("disabled","disabled"),d3.select("#domainWidthHigher").attr("disabled","disabled"),d3.select("#domainLevelLower").attr("disabled","disabled"),d3.select("#domainLevelHigher").attr("disabled","disabled"));
p();k()});d3.select("#domainColors").on("click",function(){a.showDomainColors=!a.showDomainColors;x()});d3.select("#domainNames").on("click",function(){a.showDomainNames=!a.showDomainNames;k()});d3.select("#domainWidthLower").on("click",function(){a.domainWidth-=a.domainWidthStep;a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep);I=phyd3.phylogram.scaledomainWidths(D,a.domainWidth);F()});d3.select("#domainWidthHigher").on("click",function(){a.domainWidth+=a.domainWidthStep;I=phyd3.phylogram.scaledomainWidths(D,
a.domainWidth);F()});d3.select("#domainLevelLower").on("click",function(){a.domainLevel/=a.domainLevelStep;v()});d3.select("#domainLevelHigher").on("click",function(){a.domainLevel*=a.domainLevelStep;v()});d3.select("#graphLegend").on("click",function(){a.showGraphLegend=!a.showGraphLegend;a.marginY=a.margin+(a.showGraphs?R:0);M();V()});d3.select("#graphs").on("click",function(){a.showGraphs=!a.showGraphs;a.showGraphs?(d3.select("#graphLegend").attr("disabled",null),d3.select("#graphWidthLower").attr("disabled",
null),d3.select("#graphWidthHigher").attr("disabled",null)):(d3.select("#graphLegend").attr("disabled","disabled"),d3.select("#graphWidthLower").attr("disabled","disabled"),d3.select("#graphWidthHigher").attr("disabled","disabled"));ja()});d3.select("#graphWidthLower").on("click",function(){a.graphWidth-=a.graphWidthStep;a.graphWidth<a.graphWidthStep&&(a.graphWidth=a.graphWidthStep);B()});d3.select("#graphWidthHigher").on("click",function(){a.graphWidth+=a.graphWidthStep;B()});d3.select("#linkSVG").on("click",
