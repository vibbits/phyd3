...
$jscomp.polyfill=function(d,b,a,e){if(b){a=$jscomp.global;d=d.split(".");for(e=0;e<d.length-1;e++){var h=d[e];h in a||(a[h]={});a=a[h]}d=d[d.length-1];e=a[d];b=b(e);b!=e&&null!=b&&$jscomp.defineProperty(a,d,{configurable:!0,writable:!0,value:b})}};$jscomp.polyfill("Number.MAX_SAFE_INTEGER",function(){return 9007199254740991},"es6","es3");$jscomp.SYMBOL_PREFIX="jscomp_symbol_";$jscomp.initSymbol=function(){$jscomp.initSymbol=function(){};$jscomp.global.Symbol||($jscomp.global.Symbol=$jscomp.Symbol)};
...
$jscomp.iteratorFromArray=function(d,b){$jscomp.initSymbolIterator();d instanceof String&&(d+="");var a=0,e={next:function(){if(a<d.length){var h=a++;return{value:b(h,d[h]),done:!1}}e.next=function(){return{done:!0,value:void 0}};return e.next()}};e[Symbol.iterator]=function(){return e};return e};$jscomp.polyfill("Array.prototype.values",function(d){return d?d:function(){return $jscomp.iteratorFromArray(this,function(b,a){return a})}},"es6","es3");
(function(){"undefined"==typeof phyd3&&(phyd3={});phyd3.newick={};phyd3.newick.parse=function(d){var b=[],a={};d=d.split(/\s*(;|\(|\)|,|:)\s*/);for(var e=0,h=0;h<d.length;h++){var v=d[h];switch(v){case "(":v={};a.id=e;e++;a.branchset=[v];b.push(a);a=v;break;case ",":v={};b[b.length-1].branchset.push(v);a=v;break;case ")":a=b.pop();break;case ":":break;default:var t=d[h-1];")"==t||"("==t||","==t?a.name=v:":"==t&&(a.branchLength=parseFloat(v));a.id=e;e++}}return a}})();
...
end:b[2]});!d.children&&b[2][0]>phyd3.phylogram.dx&&(phyd3.phylogram.dx=b[2][0]);return"M"+b[0]+" "+b[1]+" "+b[2]};phyd3.phylogram.scaleBranchLengths=function(d,b){function a(b,d){d(b);if(b.children)for(var e=b.children.length-1;0<=e;e--)a(b.children[e],d)}a(d[0],function(a){0>a.branchLength&&(a.branchLength*=-1);a.rootDist=(a.parent?a.parent.rootDist:0)+(a.branchLength||0)});var e=d.map(function(a){return a.rootDist}),h=d3.scale.linear().domain([0,d3.max(e)]).range([0,b]);a(d[0],function(a){a.y=
parseInt(h(a.rootDist))});return h};phyd3.phylogram.randomColor=function(){function d(a,b,d){0>d&&(d+=1);1<d&&--d;return d<1/6?a+6*(b-a)*d:.5>d?b:d<2/3?a+(b-a)*(2/3-d)*6:a}var b=Math.random();return function(a,b,h){if(0==b)h=b=a=h;else{var e=.5>h?h*(1+b):h+b-h*b,t=2*h-e;h=d(t,e,a+1/3);b=d(t,e,a);a=d(t,e,a-1/3)}return"#"+Math.round(255*h).toString(16)+Math.round(255*b).toString(16)+Math.round(255*a).toString(16)}((b+.618033988749895)%1,.5,.6)};phyd3.phylogram.scaledomainWidths=function(d,b){d=d.map(function(a){return a.sequences&&
a.sequences[0]&&a.sequences[0].domainArchitecture&&a.sequences[0].domainArchitecture.sequenceLength?a.sequences[0].domainArchitecture.sequenceLength:0});return d3.scale.linear().domain([0,d3.max(d)]).range([0,b])};phyd3.phylogram.build=function(d,b,a){function e(){n.selectAll("text.groupLabel").remove();n.selectAll("rect.groupLabel").remove();var f=0;for(gid in b.groups){var m=b.groups[gid];m.x1=Number.MAX_SAFE_INTEGER;m.x2=0;m.y1=Number.MAX_SAFE_INTEGER;m.y2=0;f<m.depth&&(f=m.depth);v(b,m)}for(;0<=
f;f--)for(gid in b.groups)m=b.groups[gid],m.depth==f&&(n.insert("text",":first-child").attr("class","groupLabel").attr("y",m.x1).attr("x",0).attr("dy",2*a.nodeHeight).attr("text-anchor","start").attr("stroke",m.foregroundColor).attr("stroke-width",a.outline+"px").attr("font-size",1.5*a.nodeHeight+"px").text(m.label),n.insert("rect",":first-child").attr("class","groupLabel").attr("stroke",m.backgroundColor).attr("fill",m.backgroundColor).attr("y",m.x1).attr("height",m.x2-m.x1).attr("x",0).attr("width",
phyd3.phylogram.dx).append("title").text(m.label))}function h(a,m,g){if(g||a.id==m){g=!0;a.groupIDs||(a.groupIDs=[]);a.groupIDs.push(m);var f=b.groups[m];a.x<f.x1&&(f.x1=a.x);a.x>f.x2&&(f.x2=a.x);a.y<f.y1&&(f.y1=a.y);a.y>f.y2&&(f.y2=a.y)}if(a.children)for(f=0;f<a.children.length;f++)h(a.children[f],m,g)}function v(a,b){if(a.groupIDs)for(var f=0;f<a.groupIDs.length;f++)a.groupIDs[f]==b.id&&(a.id==b.id&&(b.depth=a.depth),a.x<b.x1&&(b.x1=a.x),a.x>b.x2&&(b.x2=a.x),a.y<b.y1&&(b.y1=a.y),a.y>b.y2&&(b.y2=
a.y));if(a.children)for(f=0;f<a.children.length;f++)v(a.children[f],b)}function t(a,b){if(a.groupIDs){var f=a.groupIDs.indexOf(b);-1!=f&&a.groupIDs.splice(f,1)}if(a.children)for(f=0;f<a.children.length;f++)t(a.children[f],b)}function z(){a.domainWidth=a.domainWidthStep;a.graphWidth=a.initialGraphWidth;a.scaleX=1;a.scaleY=1;a.translateX=0;a.translateY=0;ea.translate([0,0])}function B(){d3.select(d).select("svg").remove();svg=d3.select(d).insert("svg");svg.attr("width",aa+"px").attr("height",a.height+
...
a.backgroundColor:a.foregroundColor}function ja(){d3.selectAll(".canvas").attr("fill",a.invertColors?a.foregroundColor:a.backgroundColor);var f=A();n.selectAll("path.link").attr("stroke",f);d3.selectAll("text").attr("fill",f);d3.selectAll("marker").selectAll("line").attr("stroke",f);d3.selectAll("text.title").attr("stroke",f);n.selectAll("text.nodelabel").attr("stroke",f).attr("fill",f);C.selectAll("path").attr("stroke",f);C.selectAll("line").attr("stroke",f);C.selectAll("rect").attr("stroke",f);
Y();ka()}function va(){var f=d.replace("#","");f=document.getElementById(f).querySelector("svg");var b=f.getElementById("main"),g=b.getBBox();b=b.getAttributeNode("transform");var e=f.getAttributeNode("width"),q=f.getAttributeNode("height"),l=b.value,k=e.value,x=q.value;b.value="translate("+a.marginX+", "+a.marginY+")";e.value=(Math.max(+e.value.replace("px",""),g.width)+a.marginX).toString();q.value=(Math.max(+q.value.replace("px",""),g.height)+a.marginY).toString();if("undefined"!=typeof window.XMLSerializer)var p=
(new XMLSerializer).serializeToString(f);else"undefined"!=typeof f.xml&&(p=f.xml);b.value=l;e.value=k;q.value=x;return p}function pa(){a.showDomains?(P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth),C.append("svg:path").attr("class","domain").attr("stroke",A()).attr("visibility",function(a,b){return a.show?"visibile":"hidden"}),I=I.data(function(a,b,g){a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.domains:[];for(g=0;g<a.length;g++)a[g].i=b;
a.sort(function(a,f){return Math.abs(f.to-f.from)-Math.abs(a.to-a.from)});return a}).enter().append("svg:g").attr("class","domain"),I.append("svg:rect").attr("class","domain hover-visible").attr("stroke",A()).attr("stroke-width",a.outline+"px").append("title").text(function(a){return a.name}),ca(),M(),ka()):(I.remove(),C.selectAll("path.domain").remove(),I=C.selectAll("g.domain"))}function fa(){a.showDomainNames?(I.append("svg:text").attr("class","domain").attr("dy",-3).attr("fill",A()).attr("font-size",
1.5*a.nodeHeight+"px").text(function(a){return a.name}),M()):I.selectAll("text.domain").remove()}function ca(){var f=C.data();C.selectAll("g.domain").attr("visibility",function(b,g,d){return f[d].show&&b.confidence<=a.domainLevel?"visible":"hidden"});C.selectAll("path.domain").attr("visibility",function(a){return a.show?"visible":"hidden"});d3.select("#domainLevel").attr("value",a.domainLevel.toPrecision(1))}function ka(){I.selectAll("rect.domain").attr("fill",function(f){return a.showDomainColors&&
b.domcolors[f.name]?b.domcolors[f.name].color.replace(/0x/,"#"):A()})}function M(){var f=C.data(),b=Q+(a.showLabels?R:0)+(a.showGraphs?w:0);P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth);C.selectAll("path.domain").attr("d",function(a,d,m){d=phyd3.phylogram.dx-f[m].y;a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.sequenceLength:0;return"M"+parseInt(b+d)+",0L"+parseInt(b+d+P(a))+",0"});I.selectAll("rect.domain").attr("width",function(a){return parseInt(P(a.to-
a.from))}).attr("transform",function(g,d,m){return"translate("+parseInt(b+(phyd3.phylogram.dx-f[g.i].y)+P(g.from))+","+parseInt(-1*a.nodeHeight)+")"}).attr("height",2*a.nodeHeight-2);I.selectAll("text.domain").attr("transform",function(g){return"translate("+parseInt(b+(phyd3.phylogram.dx-f[g.i].y)+P(g.from))+","+parseInt(-1*a.nodeHeight)+")"});d3.select("#domainWidth").attr("value",a.domainWidth)}function ha(){n.selectAll("text.supportValue").remove();a.showSupportValues&&(n.selectAll("g.inner.node").append("svg:text").attr("class",
"supportValue").attr("dx",-2).attr("text-anchor","end").attr("fill",a.supportValuesColor).text(function(f){var b="";if(f.confidences)for(var g=0;g<f.confidences.length;g++)b+=parseFloat(f.confidences[g].value).toFixed(a.maxDecimalsSupportValues)+" ";return b}),d3.select("#maxDecimalsSupportValues").attr("value",a.maxDecimalsSupportValues),F(),L())}function ia(){n.selectAll("text.branchLength").remove();a.showLengthValues&&(n.selectAll("g.node").append("svg:text").attr("class","branchLength").attr("dx",
-2).attr("text-anchor","end").attr("fill",a.branchLengthColor).text(function(f){return f.branchLength?parseFloat(f.branchLength).toFixed(a.maxDecimalsLengthValues):""}),d3.select("#maxDecimalsLengthValues").attr("value",a.maxDecimalsLengthValues),F(),L())}function la(a,b,g){var f=a.attr("id")+"_"+a.selectAll("div.panel-default").size(),d=a.append("div").attr("class","panel panel-default");d.append("div").attr("class","panel-heading").attr("role","tab").attr("id","heading"+f).append("h4").attr("class",
"panel-title").append("a").attr("class","collapsed").attr("aria-expanded",g?"true":"false").attr("role","button").attr("data-toggle","collapse").attr("data-parent","#"+a.attr("id")).attr("href","#collapse"+f).attr("aria-controls","collapse"+f).html(b);return d.append("div").attr("id","collapse"+f).attr("class","panel-collapse collapse "+(g?"in":"")).attr("role","tabpanel").attr("aria-labelledby","heading"+f).append("div").attr("class","panel-body table-responsive")}function Ba(f){var m=d3.event,g=
m.layerX;y=m.layerY;var e="popup"+parseInt(Date.now()*Math.random()*1E3),q=function(){d3.select("#"+e).remove();n.selectAll("g.node.cid_"+f.id).selectAll("rect.pointer").style("opacity","0")};g=d3.select(d).append("div").attr("class","popup").attr("id",e).style("position","absolute").style("top",parseInt(y)+"px").style("left",parseInt(g)+"px").style("width",parseInt(a.popupWidth)+"px").style("color","black").style("background-color","white");var l=g.append("button").attr("class","btn btn-link").style("position",
"absolute").style("right","8px");l.append("span").attr("class","glyphicon glyphicon-remove").attr("aria-hidden","true");l.on("click",q);g.append("span").append("a").text(function(){return-1==a.pinnedNodes.indexOf(f.id)?"Pin this node":"Unpin this node"}).on("click",function(){var b=a.pinnedNodes.indexOf(f.id);-1==b?a.pinnedNodes.push(f.id):a.pinnedNodes.splice(b,1);q();Y()});g=g.append("div").attr("class","panel-group").attr("id","accordion_"+parseInt(Date.now()*Math.random()*1E3)).attr("role","tablist").attr("aria-multiselectable",
"true");l="text"==m.target.nodeName||m.target.classList&&m.target.classList.contains("pointer")?!0:!1;l=la(g,"Node",l).append("table").attr("class","table table-condensed table-bordered").append("tbody");if(f.name){var k=l.append("tr");k.append("td").text("Name");k.append("td").text(f.name)}if(f.taxonomies)for(var x=0;x<f.taxonomies.length;x++){var p=f.taxonomies[x];k=l.append("tr");k.append("td").text("Taxononomy");k=k.append("td");b.taxcolors[p.code]&&b.taxcolors[p.code].url&&(k=k.append("a").attr("href",
b.taxcolors[p.code].url).attr("target","_blank"));var h="";h+=b.taxcolors[p.code]&&b.taxcolors[p.code].name?b.taxcolors[p.code].name+" ":"";h+=" ";h+=p.scientificName?"\x3ci\x3e"+p.scientificName+"\x3c/i\x3e":"";h+=" ";h+=p.commonName?p.commonName:"";h+=" ";h+=p.code?"("+p.code+")":"";k.html(h)}if(f.confidences)for(x=0;x<f.confidences.length;x++)k=l.append("tr"),k.append("td").text("Confidence"),k.append("td").html(f.confidences[x].value);k=l.append("tr");k.append("td").text("Depth");k.append("td").text(f.depth);
k=l.append("tr");k.append("td").text("Branch length");k.append("td").text(f.branchLength?f.branchLength:0);k=l.append("tr");k.append("td").text("Distance from root");k.append("td").text(f.rootDist);for(x=0;x<f.properties.length;x++)k=f.properties[x],u=k.ref,p=k.value,p=p.trim(),/^http:\/\//.test(p)&&(p="\x3ca href\x3d'"+p+"' target\x3d'_blank'\x3e"+p+"\x3c/a\x3e\x3cbr /\x3e"),k=l.append("tr"),k.append("td").text(u),k.append("td").html(p);for(var u in f.events)p=f.events[u],k=l.append("tr"),k.append("td").text(u),
k.append("td").html(p);if(f.sequences)for(x=0;x<f.sequences.length;x++){u=f.sequences[x];p="";p+=u.symbol?u.symbol+" ":"";p+=u.name?u.name+" ":"";p+=u.location?u.location+" ":"";p+=u.geneName?u.geneName:"";p.length&&(k=l.append("tr"),k.append("td").text("Sequence"),k.append("td").html(p));u.accession&&(p="",p+=u.accession.value?u.accession.value+" ":"",p+=u.accession.source?"["+u.accession.source+"] ":"",p+=u.accession.comment?"("+u.accession.comment+")":"",p.length&&(k=l.append("tr"),k.append("td").text("Accession"),
k.append("td").html(p)));u.molSeq&&u.molSeq.value&&(k=l.append("tr"),k.append("td").text("Molecular Sequence"),k.append("td").html(u.molSeq.value));if(u.uris){p="";for(k=0;k<u.uris.length;k++)p=u.uris[k],p="\x3ca href\x3d'"+p.value+"'\x3e"+(p.desc?p.desc:p.value)+"\x3c/a\x3e";p.length&&(k=l.append("tr"),k.append("td").text("See also"),k.append("td").html(p))}if(f.sequences[x].domainArchitecture&&f.sequences[x].domainArchitecture.domains){l=m.target.parentNode.classList&&m.target.parentNode.classList.contains("domain")?
!0:!1;l=la(g,"Domains",l).append("table").attr("id","dt"+e).attr("class","table table-condensed table-bordered");u=l.append("thead").append("tr");u.append("th").text("#");u.append("th").text("Name");u.append("th").text("Description");u.append("th").text("From");u.append("th").text("To");u.append("th").text("P value");h=l.append("tbody");for(p=0;p<f.sequences[x].domainArchitecture.domains.length;p++){var G=f.sequences[x].domainArchitecture.domains[p];k=h.append("tr");k.append("td").style("background",
b.domcolors[G.name]&&b.domcolors[G.name].color?b.domcolors[G.name].color.replace("0x","#"):"").text(" ");u=k.append("td");b.domcolors[G.name]&&b.domcolors[G.name].url&&(u=u.append("a").attr("href",b.domcolors[G.name].url).attr("target","_blank"));u.text(G.name);k.append("td").text(b.domcolors[G.name]?b.domcolors[G.name].description:"");k.append("td").text(G.from);k.append("td").text(G.to);k.append("td").text(G.confidence)}jQuery("#dt"+e).dataTable({searching:!1,paging:!1,language:{info:""},order:[[3,
"asc"]],columns:[{orderable:!1},null,null,null,null,null]})}}if(b.graphs)for(x=0;x<b.graphs.length;x++)if(h=b.graphs[x],h.data&&h.data[f.id]&&h.data[f.id].length)for(l=m.target.parentNode.classList&&m.target.parentNode.classList.contains(h.type)?!0:!1,l=la(g,h.name,l).append("table").attr("class","table table-condensed table-bordered").append("tbody"),p=0;p<h.legend.fields.length;p++)k=l.append("tr"),u=k.append("td"),h.legend.fields[p].url&&(u=u.append("a").attr("href",h.legend.fields[p].url).attr("target",
"_blank")),u.text(h.legend.fields[p].name),k.append("td").text(h.data[f.id][p]);n.selectAll("g.node.cid_"+f.id).selectAll("rect.pointer").style("opacity","1");m.stopPropagation()}function ra(){a.lineupNodes?n.selectAll("g.leaf").append("path").attr("class","support").attr("stroke",A()).attr("stroke-dasharray","2,3").attr("stroke-width","0.5px"):n.selectAll("path.support").remove()}function Z(f,d){var g="";if(a.showTaxonomy&&f.taxonomies)for(var m=0;m<f.taxonomies.length;m++){var e=f.taxonomies[m];
a.showFullTaxonomy&&(g+=b.taxcolors[e.code]&&b.taxcolors[e.code].name?b.taxcolors[e.code].name:"",g+=" ",g+=e.scientificName?e.scientificName:"",g+=" ",g+=e.commonName?e.commonName:"",g+=" ");g+=e.code?e.code:"";g+=" "}m="";if(a.showSequences&&f.sequences)for(e=0;e<f.sequences.length;e++){var l=f.sequences[e];m+=l.symbol?l.symbol:"";m+=" ";m+=l.accession?l.accession.value:"";m+=" ";m+=l.name?l.name:"";m+=" ";m+=l.geneName?l.geneName:"";m+=" ";if(l.annotations)for(var k=0;k<l.annotations.length;k++)text+=
l.annotations[k].desc?l.annotations[k].desc:"",text+=" "}e=a.showNodeNames&&f.name?f.name+" ":" ";d||(f.children&&"only leaf"==a.showNodesType&&(e=" "),f.children||"only inner"!=a.showNodesType||(e=" "));return g+e+m}function U(){var f=0;ma=0;C.selectAll("text.name").text(function(a){var b=Z(a);b.length>f&&(f=b.length,ma=a.id);return b});n.selectAll("g.node.inner").selectAll("text.name").text(Z);a.showGraphs&&V();a.showDomains&&M()}function L(){n.selectAll("text.branchLength").attr("visibility",function(a){return a.show?
...
(console.log("o"),2*a.outline+"px"):a.outline+"px"}).attr("fill",function(f){var d=null;if(a.showTaxonomyColors&&b.taxcolors&&f.taxonomies)for(var g=0;g<f.taxonomies.length;g++){var e=f.taxonomies[g];b.taxcolors[e.code]&&b.taxcolors[e.code].color&&(d=b.taxcolors[e.code].color.replace(/0x/,"#"))}return d?d:A()})}function ya(){C.selectAll("text.name").attr("dx",function(b){return parseInt(a.lineupNodes?phyd3.phylogram.dx-b.y+5:5)});C.selectAll("path.support").attr("d",function(a){return"M0,0 L"+parseInt(phyd3.phylogram.dx-
...
b<a.textLength&&(b=a.textLength);Q=b;d3.select("#nodeHeight").attr("value",2*a.nodeHeight+"px");ya();a.showLabels&&na();a.showGraphs&&V();a.showDomains&&M()}function sa(){if(a.showLabels&&b.labels){for(var f=0;f<b.labels.length;f++){var d=b.labels[f];d.id||(d.id=parseInt(Date.now()*Math.random()*1E3));d.data.tag&&n.selectAll("g.node").each(function(a){var b=a[d.data.tag],f=d.data.ref;f=f?f:"value";b="object"==typeof b?b[f]:b;if("property"==d.data.tag)for(pid=0;pid<a.properties.length;pid++){var g=
a.properties[pid];if(g.ref==f){b=g.value;break}}b&&(d.data[a.id]=b)});switch(d.type){case "text":for(cid in d.data)d.data[cid]&&n.selectAll(".cid_"+cid).append("text").attr("class","nodelabel text lid"+d.id).attr("dy",3).attr("text-anchor","start").attr("stroke-width",a.outline+"px").attr("stroke",A()).attr("fill",A()).attr("font-size",1.5*a.nodeHeight+"px").text(d.data[cid]).append("title").text(d.name+": "+d.data[cid]);break;case "color":for(cid in d.data)if(d.data[cid]){var g=a.nodeHeight,e=d.data[cid].replace(/0x/,
"#");n.selectAll(".cid_"+cid).append("rect").attr("height",2*g).attr("width",2*g).attr("class","nodelabel color lid"+d.id).attr("style","fill: "+e+"; stroke: "+e).append("title").text(d.name)}}n.append("text").attr("class","labellegend lid"+d.id).attr("fill",A()).text(0!=d.showLegend?d.name:"")}wa();na()}else n.selectAll(".nodelabel").remove(),n.selectAll(".labellegend").remove(),R=0;V();M()}function na(){var f=a.nodeHeight;R=0;if(b.labels){for(var d=0;d<b.labels.length;d++){var g=b.labels[d],e=0;
n.selectAll(".nodelabel.lid"+g.id).attr("transform",function(a){var b=0,d=0;"color"==g.type&&(b=d=-f);var m=0;"text"==this.nodeName&&this.firstChild?m=this.firstChild.length*f/1.5:"rect"==this.nodeName&&(m=2*f);m>e&&(e=m);a.branchset.length?(b+=a.y,b-=f):b+=phyd3.phylogram.dx+Q;return"translate("+parseInt(b+f+R-a.y)+","+d+")"});n.selectAll("text.labellegend.lid"+g.id).attr("transform","translate("+parseInt(phyd3.phylogram.dx+R+Q+2*f)+",-10) rotate(-90)");R+=e+2*f}d=C.selectAll(".nodelabel");d.selectAll("text").attr("dy",
a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");d.selectAll("rect").attr("width",2*a.nodeHeight).attr("height",2*a.nodeHeight)}}function wa(){n.selectAll(".nodelabel").attr("visibility",function(a){return a.show?"visible":"hidden"})}function qa(){if(a.showGraphs&&b.graphs&&b.graphs.length){d3.svg.arc().innerRadius(0).outerRadius(a.nodeHeight);for(var f=d3.layout.pie().value(function(a){return a.value}).sort(null),d=w=0;d<b.graphs.length;d++){var g=b.graphs[d];g.id||(g.id=parseInt(Date.now()*
Math.random()*1E3));if(g.data.tag)var e=n.selectAll("g.node").each(function(a){var b=a[g.data.tag],f=g.data.ref;f=f?f:"value";b="object"==typeof b?b[f]:b;if("property"==g.data.tag)for(var d=0;d<a.properties.length;d++){var e=a.properties[d];if(e.ref==f){b=e.value;break}}b&&(g.data[a.id]=[],g.data[a.id][0]=b,"pie"==g.type&&g.data.max&&(g.data[a.id][1]=g.data.max-b,g.legend.fields[1]={name:g.legend.fields[0].name,color:g.legend.fields[0].background?g.legend.fields[0].background:phyd3.phylogram.randomColor(),
invert:!0}))});switch(g.type){case "pie":for(cid in g.data)if(e=cid.replace(/\./g,"\\."),g.data[cid]&&Array.isArray(g.data[cid])){var q=!1;n.select(".cid_"+e).each(function(a){0==a.branchset.length&&(q=!0)});e=n.selectAll(".cid_"+e).append("svg:g").attr("class","graph pie gid"+g.id);for(var l=[],k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k],leaf:q});l=e.selectAll("path.pie.gid"+g.id).data(f(l)).enter().append("path").attr("class","pie hover-visible gid"+g.id).attr("stroke",
A()).attr("stroke-width",a.outline+"px").attr("fill",function(a,b){return g.legend.fields?(g.legend.fields[b]||(g.legend.fields[b]={},g.legend.fields[b].name="Series "+(b+1),g.legend.fields[b].color=phyd3.phylogram.randomColor()),g.legend.fields[b].color.replace(/0x/,"#")):""}).append("title").text(function(a,b){return g.legend.fields?(g.legend.fields[b]&&g.legend.fields[b].name?g.legend.fields[b].name+": ":"")+(g.legend.fields[b].invert?g.data.max-a.data.value:a.data.value):""})}break;case "binary":for(cid in g.data)if(g.data[cid]&&
Array.isArray(g.data[cid])){e=cid.replace(/\./g,"\\.");l=[];q=!1;n.select(".cid_"+e).each(function(a){0==a.branchset.length&&(q=!0)});e=n.selectAll(".cid_"+cid).append("svg:g").attr("class","graph binary gid"+g.id);for(k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k],shape:g.legend.fields[k]?g.legend.fields[k].shape:"",leaf:q});l=e.selectAll("path.binary.gid"+g.id).data(l).enter().append("path").attr("class","binary hover-visible gid"+g.id).attr("style",function(a,
b){b=(g.legend.fields[b]?g.legend.fields[b].color:"").replace(/0x/,"#");return"fill:"+(1<=a.value?b:"none")+";stroke:"+(0<=a.value?b:"none")}).append("title").text(function(a,b){return(g.legend.fields[b]?g.legend.fields[b].name+": ":"")+a.value})}break;case "multibar":D[g.id]=[];for(k=0;k<g.legend.fields.length;k++)l=d3.max(d3.values(g.data),function(a){if(a)return a[k]}),e=d3.min(d3.values(g.data),function(a){if(a)return a[k]}),0<e&&(e=0),D[g.id][k]=d3.scale.linear().domain([e,l]);for(cid in g.data)if(g.data[cid]){e=
cid.replace(/\./g,"\\.");e=n.select(".cid_"+e).append("svg:g").attr("class","graph multibar gid"+g.id);l=[];for(k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k]});l=e.selectAll("rect.multibar.gid"+g.id).data(l).enter().append("rect").attr("class","multibar hover-visible gid"+g.id).attr("fill",function(a,b){return(g.legend.fields[b]?g.legend.fields[b].color:"").replace(/0x/,"#")}).attr("stroke",A()).attr("stroke-width",a.outline).append("title").text(function(a,
b){return(g.legend.fields[b]?g.legend.fields[b].name+": ":"")+a.value})}break;case "boxplot":E[g.id]=null;var h=0,p=Number.MAX_SAFE_INTEGER;for(k=0;5>k;k++)l=d3.max(d3.values(g.data),function(a){if(a)return a[k]}),e=d3.max(d3.values(g.data),function(a){if(a)return a[k]}),l>h&&(h=l),e<p&&(p=e);E[g.id]=d3.scale.linear().domain([p,h]);for(cid in g.data)g.data[cid]&&(e=cid.replace(/\./g,"\\."),e=n.select(".cid_"+e).append("svg:g").attr("class","graph boxplot gid"+g.id),l=[],l.push({min:g.data[cid][0],
q1:g.data[cid][1],median:g.data[cid][2],q3:g.data[cid][3],max:g.data[cid][4]}),l=e.selectAll("rect.boxplot.gid"+g.id).data(l).enter(),l.append("line").attr("class","boxplot baseline hover-visible gid"+g.id).attr("style","marker-start: url(#markerBoxplot); marker-end: url(#markerBoxplot);").attr("stroke",A()),l.append("rect").attr("class","boxplot q1 hover-visible gid"+g.id).attr("fill",(g.legend.fields[0]?g.legend.fields[0].color:"").replace(/0x/,"#")).attr("stroke",A()).attr("stroke-width",a.outline).append("title").text(function(a){return"Min: "+
...
e=d3.min(d3.values(g.data),function(a){if(a)return d3.min(a)});var t=d3.scale.quantize().domain([e,l]).range(colorbrewer[g.legend.gradient.name][g.legend.gradient.classes]);for(cid in g.data)if(g.data[cid]){e=cid.replace(/\./g,"\\.");e=n.select(".cid_"+e).append("svg:g").attr("class","graph heatmap gid"+g.id);l=[];if(g.data[cid])for(k=0;k<g.data[cid].length;k++)void 0!=g.data[cid][k]&&l.push({i:k,value:g.data[cid][k]});l=e.selectAll("rect.heatmap.gid"+g.id).data(l).enter().append("rect").attr("class",
"heatmap hover-visible gid"+g.id).attr("fill",function(b,f){return isNaN(b.value)?a.nanColor:t(b.value)}).attr("stroke",function(b,f){return isNaN(b.value)?a.nanColor:A()}).attr("stroke-width",a.outline+"px").append("title").text(function(a,b){return(g.legend.fields[b]?g.legend.fields[b].name+": ":"")+a.value})}break;default:console.log("Graph not supported "+g.type)}}xa();V()}else w=0,n.selectAll("g.graph").remove(),n.selectAll("text.legend").remove();M()}function V(){var f=a.nodeHeight,d=Q+R;C.selectAll("g.graph").attr("transform",
function(a){return"translate("+(phyd3.phylogram.dx-a.y+d)+",0)"});var g=n.selectAll("g.graph");d3.select("#graphWidth").attr("value",a.graphWidth);n.selectAll("text.legend").remove();w=0;if(b.graphs){for(var e=0;e<b.graphs.length;e++){var q=b.graphs[e];switch(q.type){case "pie":g.selectAll("path.pie.gid"+q.id).attr("d",function(a){if(0<a.value)return d3.svg.arc().innerRadius(0).outerRadius(f)(a)}).attr("transform",function(a){return" translate("+parseInt(a.data.leaf?f+w:0)+",0)"});a.showGraphs&&a.showGraphLegend&&
n.append("text").attr("class","legend").text(0!=q.legend.show?q.name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+d+w+2*f)+",-10) rotate(-90)");w+=2*f+5;break;case "binary":g.selectAll("path.binary.gid"+q.id).attr("d",function(a){var b=d3.svg.symbol().size(4*(f-2)*(f-2));b.type(q.legend.fields[a.i].shape);return b(a)}).attr("transform",function(a){return" translate("+(a.leaf?w+10+a.i*(2*f+5):0)+" ,0)"});if(a.showGraphs&&a.showGraphLegend)for(var l=0;l<q.legend.fields.length;l++)n.append("text").attr("class",
"legend").text(0!=q.legend.show?q.legend.fields[l].name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+l*(2*f+5)+2*f)+", -10) rotate(-90)");w+=(q.legend.fields.length+1)*(2*f+5)+5;break;case "multibar":for(l=0;l<q.legend.fields.length;l++)D[q.id][l]=D[q.id][l].range([0,a.graphWidth]);g.selectAll("rect.multibar.gid"+q.id).attr("height",parseInt(2*f)).attr("width",function(a,b){b=parseInt(D[q.id]&&D[q.id][a.i]?D[q.id][a.i](a.value):0);a=parseInt(D[q.id]&&D[q.id][a.i]?D[q.id][a.i](0):0);return b>
a?b-a:a-b}).attr("transform",function(b,d){d=w+b.i*(a.graphWidth+5);var g=parseInt(D[q.id]&&D[q.id][b.i]?D[q.id][b.i](b.value):0);b=parseInt(D[q.id]&&D[q.id][b.i]?D[q.id][b.i](0):0);return" translate("+parseInt(d+(b<g?b:g))+",-"+parseInt(f)+")"}).attr("x",function(a,b){});if(a.showGraphs&&a.showGraphLegend)for(l=0;l<q.legend.fields.length;l++)n.append("text").attr("class","legend").text(0!=q.legend.show?q.legend.fields[l].name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+l*(a.graphWidth+
5)+a.graphWidth/2)+",-10) rotate(-90)");w+=q.legend.fields.length*(a.graphWidth+5);break;case "boxplot":E[q.id]=E[q.id].range([0,a.graphWidth]);g.selectAll("rect.q1.boxplot.gid"+q.id).attr("height",parseInt(2*f)).attr("width",function(a){return parseInt(E[q.id](a.median)-E[q.id](a.q1))}).attr("transform",function(a,b){a=w+E[q.id](a.q1);return" translate("+parseInt(a)+",-"+parseInt(f)+")"});g.selectAll("rect.q3.boxplot.gid"+q.id).attr("height",parseInt(2*f)).attr("width",function(a){return parseInt(E[q.id](a.q3)-
E[q.id](a.median))}).attr("transform",function(a,b){a=w+E[q.id](a.median);return" translate("+parseInt(a)+",-"+parseInt(f)+")"});g.selectAll("line.baseline.boxplot.gid"+q.id).attr("y1",parseInt(f)).attr("y2",parseInt(f)).attr("x1",function(a){return E[q.id](a.min)}).attr("x2",function(a){return E[q.id](a.max)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(f)+")"});g.selectAll("line.median.boxplot.gid"+q.id).attr("y1",0).attr("y2",parseInt(2*f)).attr("x1",function(a){return E[q.id](a.median)}).attr("x2",
function(a){return E[q.id](a.median)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(f)+")"});a.showGraphs&&a.showGraphLegend&&n.append("text").attr("class","legend").text(0!=q.legend.show?q.name:"").attr("transform","translate("+(phyd3.phylogram.dx+d+w+2*f)+", -10) rotate(-90)");w+=a.graphWidth+5;break;case "heatmap":g.selectAll("rect.heatmap.gid"+q.id).attr("width",2*f).attr("height",2*f).attr("transform",function(a,b){return" translate("+(w+2*a.i*f)+",-"+f+")"});
if(a.showGraphs&&a.showGraphLegend)for(l=0;l<q.legend.fields.length;l++)n.append("text").attr("class","legend").text(0!=q.legend.show?q.legend.fields[l].name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+d+w+(l+1)*f*2)+",-10) rotate(-90)");w+=2*q.legend.fields.length*f+5}}n.selectAll("text.legend").attr("fill",A()).attr("font-size",2*a.nodeHeight+"px")}}function Ca(){var d=a.nodeHeight;w=0;if(b.graphs)for(var e=0;e<b.graphs.length;e++){var g=b.graphs[e];switch(g.type){case "pie":w+=
2*d+5;break;case "binary":w+=(g.legend.fields.length+1)*(2*d+5)+5;break;case "multibar":w+=g.legend.fields.length*(a.graphWidth+5);break;case "heatmap":w+=2*g.legend.fields.length*d+5}}return 2*a.margin+(a.showNodeNames?Q:0)+(a.showGraphs?w:0)+(a.showDomains?a.domainWidth:0)}function xa(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";for(var f=!0,g=a.nodeHeight,e=d.x,m=d.y,h=0;h<b.length;h++){var p=b[h].x,n=b[h].y;if((p-e)*(p-e)+(n-m)*(n-m)<=4*g*g){f=!1;break}}return f?(b.push(d),"visible"):
"hidden"};C.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});n.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",d);n.selectAll("g.inner").selectAll(".graph.binary").attr("visibility",d)}function O(d,m){phyd3.phylogram.lineAreas=[];phyd3.phylogram.dx=0;m=d3.layout.cluster().separation(function(a,b){return 1}).children(a.children||function(a){return a.branchset}).size([W*a.scaleY,N*a.scaleX]);K=m(b);a.showPhylogram||phyd3.phylogram.scaleBranchLengths(K,
N*a.scaleX);var f=n.selectAll("path.link");d||(f=f.data(m.links(K)),f.enter().append("svg:path").attr("class","link").attr("fill","none").attr("stroke",function(a){if(a.source.groupIDs&&a.source.groupIDs.length){for(var d={depth:0},f=0;f<a.source.groupIDs.length;f++){var e=b.groups[a.source.groupIDs[f]];e&&e.depth>=d.depth&&(d=e)}return d.foregroundColor}return A()}));f.attr("d",phyd3.phylogram.rightAngleDiagonal);H=void 0;c=0;var h=K.filter(function(a){return!a.children});for(m=0;m<h.length;m++)(f=
-1!=a.pinnedNodes.indexOf(h[m].id))?(h[m].show=!0,h[m].pinned=!0,da(h[m],!a.lineupNodes)||(H.show=H.pinned?!0:!1),H=h[m]):h[m].show=a.dynamicHide?da(h[m],!a.lineupNodes):!0;H=void 0;h=K.filter(function(a){return a.children});for(m=0;m<h.length;m++)(f=-1!=a.pinnedNodes.indexOf(h[m].id))?(h[m].show=!0,h[m].pinned=!0,da(h[m],!0)||(H.show=H.pinned?!0:!1),H=h[m]):h[m].show=a.dynamicHide?da(h[m],!0):!0;d||(T=T.data(K),T.enter().append("svg:g").attr("class",function(a){return a.children?0==a.depth?"root node cid_"+
a.id:"inner node cid_"+a.id:"leaf node cid_"+a.id}).style("cursor","pointer").on("click",function(d){var f=d3.event;!f.ctrlKey||f.altKey||f.shiftKey?!f.altKey||f.ctrlKey||f.shiftKey?!f.shiftKey||f.altKey||f.ctrlKey?f.shiftKey||f.altKey||f.ctrlKey||(void 0!==a.popupAction?a.popupAction(d):Ba(d)):(jQuery("#groupID").val(d.id),jQuery("#groupDepth").val(d.depth),b.groups[d.id]?(jQuery("#groupLabel").val(b.groups[d.id].label),jQuery("#groupLabelForegroundCP").colorpicker("setValue",b.groups[d.id].foregroundColor),
jQuery("#groupLabelBackgroundCP").colorpicker("setValue",b.groups[d.id].backgroundColor)):(jQuery("#groupLabel").val(""),jQuery("#groupLabelForegroundCP").colorpicker("setValue",A()),jQuery("#groupLabelBackgroundCP").colorpicker("setValue",a.invertColors?a.foregroundColor:a.backgroundColor)),jQuery("#groupLabelModal").modal("show")):(za(b,d.id),z(),B()):(Aa(b,d.id),e(),B())}).style("z-index",100).append("rect").attr("class","pointer").attr("fill","red").attr("width",2*a.nodeHeight+2+"px").attr("height",
2*a.nodeHeight+2+"px").style("opacity","0").attr("x","-"+(a.nodeHeight+1)+"px").attr("y","-"+(a.nodeHeight+1)+"px"));T.attr("transform",function(a){return"translate("+parseInt(a.y)+","+parseInt(a.x)+")"});phyd3.phylogram.lineAreas=[]}function ua(b,d){if(Z(b,!0).match(d)){var f=!b.children&&a.lineupNodes?phyd3.phylogram.dx-b.y:0;n.selectAll("g.node.cid_"+b.id).append("rect").attr("class","searchResults").attr("width",Q+"px").attr("height",2*a.nodeHeight+2+"px").attr("fill","yellow").style("opacity",
"0.3").attr("x",f+"px").attr("y","-"+(a.nodeHeight+1)+"px")}if(b.children)for(f=0;f<b.children.length;f++)r=ua(b.children[f],d);return!1}function Aa(a,b,d){if(d||a.id==b)d=!0,a.branchset=a.branchset.reverse();if(a.children)for(var f=0;f<a.children.length;f++)Aa(a.children[f],b,d)}function za(a,d){if(a.id==d)b.branchset=[a];else if(a.children)for(var f=0;f<a.children.length;f++)za(a.children[f],d)}function S(){O(!0);ba()}function da(b,d){var f=!0,e=b.y+1,h=b.y+5.5*Z(b,!0).length,m=b.x-a.nodeHeight,
k=b.x+a.nodeHeight;if(!H)return H=b,!0;a.lineupNodes&&!b.children&&(e+=phyd3.phylogram.dx-b.y,h+=phyd3.phylogram.dx-b.y);var n=H,p=n.y+1,t=n.y+5.5*Z(n,!0).length,u=n.x-a.nodeHeight,v=n.x+a.nodeHeight;a.lineupNodes&&!n.children&&(p+=phyd3.phylogram.dx-n.y,t+=phyd3.phylogram.dx-n.y);if(p<=e&&e<=t&&u<=m&&m<=v||p<=h&&h<=t&&u<=m&&m<=v||p<=e&&e<=t&&u<=k&&k<=v||p<=h&&h<=t&&u<=k&&k<=v)f=!1;if(f&&d)for(d=0;d<phyd3.phylogram.lineAreas.length;d++)if(u=phyd3.phylogram.lineAreas[d],n=u.start[0],p=u.end[0],t=u.start[1],
u=u.end[1],t==u){if(m<=t&&u<=k&&!(n>h||p<e)){f=!1;break}}else if(n==p&&e<=n&&p<=h&&!(t>k||u<m)){f=!1;break}f&&(H=b);return f}function ba(){var b=ea.translate(),d=b[0]+a.marginX-a.translateX,g=b[1]+a.marginY-a.translateY;n.attr("transform",function(a){return"translate("+parseInt(d)+","+parseInt(g)+") scale(1)"});0!=a.lastScale&&(ya(),a.dynamicHide&&L());ta();e()}a=a||{};a.scaleY=a.scaleY||1;a.scaleX=a.scaleX||1;a.translateX=a.translateX||0;a.translateY=a.translateY||0;a.height=a.height||800;a.margin=
a.margin||20;a.scaleStep=a.scaleStep||.3;a.nodeHeight=a.nodeHeight||6;a.nodeHeightStep=a.nodeHeightStep||1;a.textLength=a.textLength||100;a.domainWidth=a.domainWidth||100;a.domainWidthStep=a.domainWidthStep||100;a.graphWidth=a.graphWidth||20;a.graphWidthStep=a.graphWidthStep||10;a.domainLevel=a.domainLevel||1;a.domainLevelStep=a.domainLevelStep||10;a.outline=a.outline||.3;a.popupWidth=a.popupWidth||500;a.maxDecimalsSupportValues=a.maxDecimalsSupportValues||0;a.maxDecimalsLengthValues=a.maxDecimalsLengthValues||
2;a.nanColor=a.nanColor||"#fff";a.foregroundColor=a.foregroundColor||"#000";a.backgroundColor=a.backgroundColor||"#fff";a.branchLengthColor=a.branchLengthColor||"red";a.supportValuesColor=a.supportValuesColor||"blue";a.showNodesType=a.showNodesType||"only leaf";a.treeWidth=a.treeWidth||"auto";a.showFullTaxonomy="showFullTaxonomy"in a?a.showFullTaxonomy:!1;a.showLabels="showLabels"in a?a.showLabels:!0;a.showDomains="showDomains"in a?a.showDomains:!0;a.dynamicHide="dynamicHide"in a?a.dynamicHide:!1;
a.invertColors="invertColors"in a?a.invertColors:!1;a.lineupNodes="lineupNodes"in a?a.lineupNodes:!0;a.showSupportValues="showSupportValues"in a?a.showSupportValues:!1;a.showLengthValues="showLengthValues"in a?a.showLengthValues:!1;a.showTaxonomy="showTaxonomy"in a?a.showTaxonomy:!0;a.showTaxonomyColors="showTaxonomyColors"in a?a.showTaxonomyColors:!0;a.showDomainNames="showDomainNames"in a?a.showDomainNames:!1;a.showDomainColors="showDomainColors"in a?a.showDomainColors:!0;a.showGraphs="showGraps"in
a?a.showGraphs:!0;a.showGraphLegend="showGraphLegend"in a?a.showGraphLegend:!0;a.showNodeNames="showNodeNames"in a?a.showNodeNames:!0;a.showPhylogram="showPhylogram"in a?a.showPhylogram:!1;a.pinnedNodes=a.pinnedNodes||[];b.groups=b.groups?b.groups:{};var n,T,C,I,K,P,H,E=[],D=[],R=100,Q=100,w=0,X=100,ma=0,Da=jQuery.extend(!0,{},b),J=!1;if(b.graphs)for(var oa=0;oa<b.graphs.length;oa++)if(0!=b.graphs[oa].legend.show){J=!0;break}X=J?X:0;a.marginX=a.margin;a.marginY=a.margin+(a.showGraphs?X:0);a.initialGraphWidth=
a.graphWidth;var aa=jQuery(d).width(),N="auto"==a.treeWidth?aa-Ca():a.treeWidth,W=a.height-(2*a.margin+(a.showGraphs?X:0));N=parseInt(N);W=parseInt(W);0>N&&(N=aa-2*Q-(a.showGraphs?2*X:0));var ea=d3.behavior.zoom().scaleExtent([1,1]).on("zoom",function(b){b=0;var d=d3.event.sourceEvent;if(d){if(0<d.deltaY||0>d.wheelDelta)b=-1*a.scaleStep;if(0>d.deltaY||0<d.wheelDelta)b=a.scaleStep;var f=.5,e=.5;d.altKey||d.ctrlKey||(f=d.layerX/aa,e=d.layerY/a.height);if(d.ctrlKey||!d.altKey)a.scaleY+=b,a.scaleY<a.scaleStep?
a.scaleY=a.scaleStep:a.translateY+=W*b*e;if(d.altKey||!d.ctrlKey)a.graphWidth+=0==b?0:0>b?-10:10,10>a.graphWidth&&(a.graphWidth=10),a.domainWidth+=0==b?0:0>b?-1*a.domainWidthStep:a.domainWidthStep,a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep),a.scaleX+=b,a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX+=N*b*f;a.lastScale=b;0!=a.lastScale?requestAnimFrame(S):requestAnimFrame(ba)}});d3.select("#dynamicHide").on("click",function(){a.dynamicHide=!a.dynamicHide;O(!0);L()});d3.select("#invertColors").on("click",
function(){a.invertColors=!a.invertColors;ja()});jQuery("#foregroundColorButton").colorpicker().on("changeColor",function(){a.foregroundColor=jQuery("#foregroundColor").val();ja()});jQuery("#backgroundColorButton").colorpicker().on("changeColor",function(){a.backgroundColor=jQuery("#backgroundColor").val();ja()});d3.select("#nodesType").on("change",function(){a.showNodesType=jQuery(this).val();S();U()});d3.select("#phylogram").on("click",function(){a.showPhylogram=!a.showPhylogram;O(!0);F();L()});
d3.select("#lineupNodes").on("click",function(){a.lineupNodes=!a.lineupNodes;O(!0);ra();F();L()});d3.select("#lengthValues").on("click",function(){a.showLengthValues=!a.showLengthValues;a.showLengthValues?d3.select("#maxDecimalsLengthValues").attr("disabled",null):d3.select("#maxDecimalsLengthValues").attr("disabled","disabled");ia()});d3.select("#maxDecimalsLengthValues").on("change",function(){a.maxDecimalsLengthValues=jQuery(this).val();ia()});d3.select("#supportValues").on("click",function(){a.showSupportValues=
!a.showSupportValues;a.showSupportValues?d3.select("#maxDecimalsSupportValues").attr("disabled",null):d3.select("#maxDecimalsSupportValues").attr("disabled","disabled");ha()});d3.select("#maxDecimalsSupportValues").on("change",function(){a.maxDecimalsSupportValues=jQuery(this).val();ha()});d3.select("#nodeNames").on("click",function(){a.showNodeNames=!a.showNodeNames;U();F()});d3.select("#nodeLabels").on("click",function(){a.showLabels=!a.showLabels;sa()});d3.select("#taxonomy").on("click",function(){a.showTaxonomy=
!a.showTaxonomy;U();F()});d3.select("#sequences").on("click",function(){a.showSequences=!a.showSequences;U();F()});d3.select("#nodesType").on("change",function(){a.showNodesType=jQuery(this).val();S();U();F()});d3.select("#taxonomyColors").on("click",function(){a.showTaxonomyColors=!a.showTaxonomyColors;Y()});d3.select("#nodeHeightLower").on("click",function(){a.nodeHeight-=a.nodeHeightStep;a.nodeHeight<a.nodeHeightStep&&(a.nodeHeight=a.nodeHeightStep);O(!0);F();L();e()});d3.select("#nodeHeightHigher").on("click",
function(){a.nodeHeight+=a.nodeHeightStep;O(!0);F();L();e()});d3.select("#resetZoom").on("click",function(){b=jQuery.extend(!0,{},Da);z();B()});d3.select("#zoominY").on("click",function(){a.scaleY+=a.scaleStep;a.translateY+=W*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(S)});d3.select("#zoomoutY").on("click",function(){a.scaleY-=a.scaleStep;a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY-=W*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(S)});d3.select("#zoominX").on("click",
function(){a.scaleX+=a.scaleStep;a.translateX+=N*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(S)});d3.select("#zoomoutX").on("click",function(){a.scaleX-=a.scaleStep;a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX-=N*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(S)});d3.select("#domains").on("click",function(){a.showDomains=!a.showDomains;a.showDomains?(d3.select("#domainNames").attr("disabled",null),d3.select("#domainColors").attr("disabled",null),d3.select("#domainWidthLower").attr("disabled",
...
"disabled"));pa();fa()});d3.select("#domainColors").on("click",function(){a.showDomainColors=!a.showDomainColors;ka()});d3.select("#domainNames").on("click",function(){a.showDomainNames=!a.showDomainNames;fa()});d3.select("#domainWidthLower").on("click",function(){a.domainWidth-=a.domainWidthStep;a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep);P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth);M()});d3.select("#domainWidthHigher").on("click",function(){a.domainWidth+=a.domainWidthStep;
P=phyd3.phylogram.scaledomainWidths(K,a.domainWidth);M()});d3.select("#domainLevelLower").on("click",function(){a.domainLevel/=a.domainLevelStep;ca()});d3.select("#domainLevelHigher").on("click",function(){a.domainLevel*=a.domainLevelStep;ca()});d3.select("#graphLegend").on("click",function(){a.showGraphLegend=!a.showGraphLegend;a.marginY=a.margin+(a.showGraphs?X:0);V();ba()});d3.select("#graphs").on("click",function(){a.showGraphs=!a.showGraphs;a.showGraphs?(d3.select("#graphLegend").attr("disabled",
null),d3.select("#graphWidthLower").attr("disabled",null),d3.select("#graphWidthHigher").attr("disabled",null)):(d3.select("#graphLegend").attr("disabled","disabled"),d3.select("#graphWidthLower").attr("disabled","disabled"),d3.select("#graphWidthHigher").attr("disabled","disabled"));qa()});d3.select("#graphWidthLower").on("click",function(){a.graphWidth-=a.graphWidthStep;a.graphWidth<a.graphWidthStep&&(a.graphWidth=a.graphWidthStep);F()});d3.select("#graphWidthHigher").on("click",function(){a.graphWidth+=
a.graphWidthStep;F()});d3.select("#linkSVG").on("click",function(){var a=va();saveAs(new Blob([unescape(encodeURIComponent(a))],{type:"application/svg+xml"}),"phylogram.svg")});d3.select("#linkPNG").on("click",function(){var a=va(),b=document.createElement("canvas");d3.select("g#main").node().getBBox();canvg(b,a);b.toBlob(function(a){saveAs(a,"phylogram.png")})});d3.select("#searchQuery").on("change",function(){ta()});d3.select("#applyTaxonomyColors").on("click",function(){d3.selectAll("input.taxColor").each(function(a){b.taxcolors[a.taxonomy].color=
jQuery(this).val()});jQuery("#taxonomyColorsModal").modal("hide");Y()});d3.select("#applyGroupLabel").on("click",function(){var a=jQuery("#groupID").val(),d=jQuery("#groupDepth").val(),g=jQuery("#groupLabel").val(),n=jQuery("#groupLabelBackground").val(),q=jQuery("#groupLabelForeground").val();jQuery("#groupLabelModal").modal("hide");b.groups[a]={id:a,depth:d,label:g,foregroundColor:q,backgroundColor:n};h(b,a);B();e()});d3.select("#clearGroupLabel").on("click",function(){var a=jQuery("#groupID").val();
jQuery("#groupLabelModal").modal("hide");delete b.groups[a];t(b,a);B()});J=function(){var a=[],d;for(d in b.taxcolors){var e=b.taxcolors[d];e.taxonomy=d;a.push(e)}return a}();J.length?(J=d3.select("#taxonomyColorsList").selectAll("div.taxColor").data(J).enter().append("div").attr("class","form-group col-xs-4 taxColor"),J.append("label").attr("class","col-xs-4 control-label").text(function(a){return a.taxonomy}),J=J.append("div").attr("class","col-xs-8").append("div").attr("class","input-group colorpicker-component"),
J.append("input").attr("class","form-control taxColor").attr("type","text").attr("value",function(a){return a.color.replace("0x","#")}),J.append("span").attr("class","input-group-btn").append("span").attr("class","input-group-addon btn btn-fab btn-fab-mini").append("i"),jQuery(".colorpicker-component").colorpicker(),jQuery("#taxonomyColorsModal").on("hide.bs.modal",function(){jQuery(".colorpicker-component").colorpicker("hide")}),jQuery("#groupLabelModal").on("hide.bs.modal",function(){jQuery(".colorpicker-component").colorpicker("hide")})):
d3.select("#taxonomyColorsList").append("div").attr("class","col-sm-12 text-center").text("No taxonomy information available");B()}})();
...
{show:parseInt(d.getAttribute("show")),fields:[],gradient:{}},a=d.getElementsByTagName("field"),e=0;e<a.length;e++){var h=a[e],v=h.getElementsByTagName("name")[0];v=v?v.textContent:"";var t=h.getElementsByTagName("color")[0];t=t?t.textContent:"";var z=h.getElementsByTagName("background")[0];z=z?z.textContent:"";var B=h.getElementsByTagName("shape")[0];B=B?B.textContent:"";h=(h=h.getElementsByTagName("url")[0])?h.textContent:"";b.fields.push({name:v,color:t,background:z,shape:B,url:h})}if(d=d.getElementsByTagName("gradient")[0])v=
(v=d.getElementsByTagName("name")[0])?v.textContent:"",d=(d=d.getElementsByTagName("classes")[0])?parseInt(d.textContent):0,b.gradient={name:v,classes:d};return b};phyd3.phyloxml.parseData=function(d){var b={tag:d.getAttribute("tag"),ref:d.getAttribute("ref")},a=d.getElementsByTagName("min")[0];b.min=a?a.textContent:void 0;a=d.getElementsByTagName("max")[0];b.max=a?a.textContent:void 0;d=d.getElementsByTagName("values");for(a=0;a<d.length;a++){var e=d[a].getAttribute("for"),h=d[a].getElementsByTagName("value");
b[e]=[];for(var v=0;v<h.length;v++)b[e].push(parseFloat(h[v].textContent))}return b};phyd3.phyloxml.parseGraph=function(d){for(var b={name:"",legend:{},data:{},type:d.getAttribute("type")},a=0;a<d.childNodes.length;a++){var e=d.childNodes[a];switch(e.nodeName){case "name":b.name=e.textContent;break;case "legend":b.legend=phyd3.phyloxml.parseLegend(e);break;case "data":b.data=phyd3.phyloxml.parseData(e);break;case "#text":case "#comment":break;default:console.log("Undefined tag: "+e.nodeName+" "+e.textContent+
...
{branchset:[],properties:[],confidence:[]},a=d.getElementsByTagName("phylogeny");a=a[0];b.rooted=a.getAttribute("rooted");b.rerootable=a.getAttribute("rerootable");b.branch_length_unit=a.getAttribute("branch_length_unit");b.type=a.getAttribute("type");if(a&&a.childNodes)for(var e=0;e<a.childNodes.length;e++){var h=a.childNodes[e];switch(h.nodeName){case "clade":b.branchset.push(phyd3.phyloxml.parseClade(h));break;case "confidence":b.confidence.push(phyd3.phyloxml.parseConfidence(h));break;case "date":b.date=
h.textContent;break;case "description":b.description=h.textContent;break;case "id":b.id=h.textContent;break;case "name":b.name=h.textContent;break;case "property":b.properties.push(phyd3.phyloxml.parseProperty(h));break;case "#text":case "#comment":break;default:console.log("Undefined tag: "+h.nodeName+" "+h.textContent+" - skipping...")}}a=d.getElementsByTagName("taxonomies");a=a[0];var v={};if(a&&a.childNodes)for(e=0;e<a.childNodes.length;e++){var t=a.childNodes[e];if("color"==t.nodeName){h=t.getElementsByTagName("code")[0].textContent;
var z=t.getElementsByTagName("value")[0].textContent;v[h]={color:z}}if("taxonomy"==t.nodeName){h=t.getAttribute("code");z=(z=t.getElementsByTagName("color")[0])?z.textContent:z;var B=t.getElementsByTagName("name")[0];B=B?B.textContent:B;t=(t=t.getElementsByTagName("url")[0])?t.textContent:t;v[h]={color:z,name:B,url:t}}}b.taxcolors=v;a=d.getElementsByTagName("domains");a=a[0];v={};if(a&&a.childNodes)for(e=0;e<a.childNodes.length;e++)t=a.childNodes[e],"color"==t.nodeName&&(h=t.getElementsByTagName("code")[0].textContent,
z=t.getElementsByTagName("value")[0].textContent,v[h]={color:z}),"domain"==t.nodeName&&(h=t.getAttribute("name"),z=(z=t.getElementsByTagName("color")[0])?z.textContent:z,B=(B=t.getElementsByTagName("description")[0])?B.textContent:B,t=(t=t.getElementsByTagName("url")[0])?t.textContent:t,v[h]={color:z,description:B,url:t});b.domcolors=v;a=d.getElementsByTagName("graphs");a=a[0];h=[];if(a&&a.childNodes)for(e=0;e<a.childNodes.length;e++)z=a.childNodes[e],"graph"==z.nodeName&&h.push(phyd3.phyloxml.parseGraph(z));
b.graphs=h;a=d.getElementsByTagName("labels");a=a[0];d=[];if(a&&a.childNodes)for(e=0;e<a.childNodes.length;e++)h=a.childNodes[e],"label"==h.nodeName&&d.push(phyd3.phyloxml.parseLabel(h));b.labels=d;return b}})();
/* Blob.js
