...
(function(){"undefined"==typeof phyd3&&(phyd3={});phyd3.newick={};phyd3.newick.parse=function(d){var b=[],a={};d=d.split(/\s*(;|\(|\)|,|:)\s*/);for(var f=0,h=0;h<d.length;h++){var v=d[h];switch(v){case "(":v={};a.id=f;f++;a.branchset=[v];b.push(a);a=v;break;case ",":v={};b[b.length-1].branchset.push(v);a=v;break;case ")":a=b.pop();break;case ":":break;default:var q=d[h-1];")"==q||"("==q||","==q?a.name=v:":"==q&&(a.branchLength=parseFloat(v));a.id=f;f++}}return a}})();
...
parseInt(h(a.rootDist))});return h};phyd3.phylogram.randomColor=function(){function d(a,b,d){0>d&&(d+=1);1<d&&--d;return d<1/6?a+6*(b-a)*d:.5>d?b:d<2/3?a+(b-a)*(2/3-d)*6:a}var b=Math.random();return function(a,b,h){if(0==b)h=b=a=h;else{var f=.5>h?h*(1+b):h+b-h*b,q=2*h-f;h=d(q,f,a+1/3);b=d(q,f,a);a=d(q,f,a-1/3)}return"#"+Math.round(255*h).toString(16)+Math.round(255*b).toString(16)+Math.round(255*a).toString(16)}((b+.618033988749895)%1,.5,.6)};phyd3.phylogram.scaledomainWidths=function(d,b){d=d.map(function(a){return a.sequences&&
a.sequences[0]&&a.sequences[0].domainArchitecture&&a.sequences[0].domainArchitecture.sequenceLength?a.sequences[0].domainArchitecture.sequenceLength:0});return d3.scale.linear().domain([0,d3.max(d)]).range([0,b])};phyd3.phylogram.build=function(d,b,a){function f(){m.selectAll("text.groupLabel").remove();m.selectAll("rect.groupLabel").remove();var g=0;for(gid in b.groups){var d=b.groups[gid];d.x1=Number.MAX_SAFE_INTEGER;d.x2=0;d.y1=Number.MAX_SAFE_INTEGER;d.y2=0;g<d.depth&&(g=d.depth);v(b,d)}for(;0<=
g;g--)for(gid in b.groups)d=b.groups[gid],d.depth==g&&(m.insert("text",":first-child").attr("class","groupLabel").attr("y",d.x1).attr("x",0).attr("dy",2*a.nodeHeight).attr("text-anchor","start").attr("stroke",d.foregroundColor).attr("stroke-width",a.outline+"px").attr("font-size",1.5*a.nodeHeight+"px").text(d.label),m.insert("rect",":first-child").attr("class","groupLabel").attr("stroke",d.backgroundColor).attr("fill",d.backgroundColor).attr("y",d.x1).attr("height",d.x2-d.x1).attr("x",0).attr("width",
phyd3.phylogram.dx).append("title").text(d.label))}function h(a,d,e){if(e||a.id==d){e=!0;a.groupIDs||(a.groupIDs=[]);a.groupIDs.push(d);var g=b.groups[d];a.x<g.x1&&(g.x1=a.x);a.x>g.x2&&(g.x2=a.x);a.y<g.y1&&(g.y1=a.y);a.y>g.y2&&(g.y2=a.y)}if(a.children)for(g=0;g<a.children.length;g++)h(a.children[g],d,e)}function v(a,b){if(a.groupIDs)for(var g=0;g<a.groupIDs.length;g++)a.groupIDs[g]==b.id&&(a.id==b.id&&(b.depth=a.depth),a.x<b.x1&&(b.x1=a.x),a.x>b.x2&&(b.x2=a.x),a.y<b.y1&&(b.y1=a.y),a.y>b.y2&&(b.y2=
...
"10").attr("stroke",z()).attr("stroke-width","2px");m=svg.append("svg:g",":first-child").attr("id","main").attr("transform","translate("+a.marginX+", "+a.marginY+")");svg.append("svg:text").attr("dy","20px").attr("class","title").attr("stroke",z()).attr("stroke-width","1px").attr("font-size","15px").text(b.name?b.name:"");svg.append("svg:text").attr("class","title").attr("dy","40px").attr("stroke",z()).attr("stroke-width","0.5px").attr("font-size","10px").text(b.description?b.description:"");R=m.selectAll("g.node");
M();0==phyd3.phylogram.dx&&(d3.select("#phylogram").attr("checked","").attr("disabled","disabled"),a.showPhylogram=!0,M());B=m.selectAll("g.leaf.node");G=B.selectAll("g.domain");ma();da();na();ea();fa();oa();pa();R.append("svg:text").attr("class","name").attr("dx",a.nodeHeight+1).attr("dy",3).attr("text-anchor","start");Z();S();J();aa();E()}function qa(){m.selectAll("rect.searchResults").remove();var a=jQuery("#searchQuery").val(),d=new RegExp(a,"i");a&&a.length&&ra(b,d)}function z(){return a.invertColors?
a.backgroundColor:a.foregroundColor}function ga(){d3.selectAll(".canvas").attr("fill",a.invertColors?a.foregroundColor:a.backgroundColor);var g=z();m.selectAll("path.link").attr("stroke",g);d3.selectAll("text").attr("fill",g);d3.selectAll("marker").selectAll("line").attr("stroke",g);d3.selectAll("text.title").attr("stroke",g);m.selectAll("text.nodelabel").attr("stroke",g).attr("fill",g);B.selectAll("path").attr("stroke",g);B.selectAll("line").attr("stroke",g);B.selectAll("rect").attr("stroke",g);
Z();ha()}function sa(){var g=d.replace("#",""),g=document.getElementById(g).querySelector("svg"),b=g.getElementById("main"),e=b.getBBox(),b=b.getAttributeNode("transform"),u=g.getAttributeNode("width"),l=g.getAttributeNode("height"),k=b.value,p=u.value,f=l.value;b.value="translate("+a.marginX+", "+a.marginY+")";u.value=(Math.max(+u.value.replace("px",""),e.width)+a.marginX).toString();l.value=(Math.max(+l.value.replace("px",""),e.height)+a.marginY).toString();if("undefined"!=typeof window.XMLSerializer)var h=
(new XMLSerializer).serializeToString(g);else"undefined"!=typeof g.xml&&(h=g.xml);b.value=k;u.value=p;l.value=f;return h}function ma(){a.showDomains?(N=phyd3.phylogram.scaledomainWidths(I,a.domainWidth),B.append("svg:path").attr("class","domain").attr("stroke",z()).attr("visibility",function(a,b){return a.show?"visibile":"hidden"}),G=G.data(function(a,b,e){a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.domains:[];for(e=0;e<a.length;e++)a[e].i=b;
a.sort(function(a,g){return Math.abs(g.to-g.from)-Math.abs(a.to-a.from)});return a}).enter().append("svg:g").attr("class","domain"),G.append("svg:rect").attr("class","domain hover-visible").attr("stroke",z()).attr("stroke-width",a.outline+"px").append("title").text(function(a){return a.name}),ba(),K(),ha()):(G.remove(),B.selectAll("path.domain").remove(),G=B.selectAll("g.domain"))}function da(){a.showDomainNames?(G.append("svg:text").attr("class","domain").attr("dy",-3).attr("fill",z()).attr("font-size",
1.5*a.nodeHeight+"px").text(function(a){return a.name}),K()):G.selectAll("text.domain").remove()}function ba(){var g=B.data();B.selectAll("g.domain").attr("visibility",function(b,e,d){return g[d].show&&b.confidence<=a.domainLevel?"visible":"hidden"});B.selectAll("path.domain").attr("visibility",function(a){return a.show?"visible":"hidden"});d3.select("#domainLevel").attr("value",a.domainLevel.toPrecision(1))}function ha(){G.selectAll("rect.domain").attr("fill",function(g){return a.showDomainColors&&
...
a.from))}).attr("transform",function(e,d,l){return"translate("+parseInt(b+(phyd3.phylogram.dx-g[e.i].y)+N(e.from))+","+parseInt(-1*a.nodeHeight)+")"}).attr("height",2*a.nodeHeight-2);G.selectAll("text.domain").attr("transform",function(e){return"translate("+parseInt(b+(phyd3.phylogram.dx-g[e.i].y)+N(e.from))+","+parseInt(-1*a.nodeHeight)+")"});d3.select("#domainWidth").attr("value",a.domainWidth)}function ea(){m.selectAll("text.supportValue").remove();a.showSupportValues&&(m.selectAll("g.inner.node").append("svg:text").attr("class",
"supportValue").attr("dx",-2).attr("text-anchor","end").attr("fill",a.supportValuesColor).text(function(g){var b="";if(g.confidences)for(var e=0;e<g.confidences.length;e++)b+=parseFloat(g.confidences[e].value).toFixed(a.maxDecimalsSupportValues)+" ";return b}),d3.select("#maxDecimalsSupportValues").attr("value",a.maxDecimalsSupportValues),E(),J())}function fa(){m.selectAll("text.branchLength").remove();a.showLengthValues&&(m.selectAll("g.node").append("svg:text").attr("class","branchLength").attr("dx",
-2).attr("text-anchor","end").attr("fill",a.branchLengthColor).text(function(g){return g.branchLength?parseFloat(g.branchLength).toFixed(a.maxDecimalsLengthValues):""}),d3.select("#maxDecimalsLengthValues").attr("value",a.maxDecimalsLengthValues),E(),J())}function ia(a,b,e){var g=a.attr("id")+"_"+a.selectAll("div.panel-default").size(),d=a.append("div").attr("class","panel panel-default");d.append("div").attr("class","panel-heading").attr("role","tab").attr("id","heading"+g).append("h4").attr("class",
"panel-title").append("a").attr("class","collapsed").attr("aria-expanded",e?"true":"false").attr("role","button").attr("data-toggle","collapse").attr("data-parent","#"+a.attr("id")).attr("href","#collapse"+g).attr("aria-controls","collapse"+g).html(b);return d.append("div").attr("id","collapse"+g).attr("class","panel-collapse collapse "+(e?"in":"")).attr("role","tabpanel").attr("aria-labelledby","heading"+g).append("div").attr("class","panel-body table-responsive")}function za(g){var f=d3.event,e=
...
"true");l.on("click",function(){d3.select("#"+u).remove();m.selectAll("g.node.cid_"+g.id).selectAll("rect.pointer").style("opacity","0")});e=e.append("div").attr("class","panel-group").attr("id","accordion_"+parseInt(Date.now()*Math.random()*1E3)).attr("role","tablist").attr("aria-multiselectable","true");l="text"==f.target.nodeName||f.target.classList&&f.target.classList.contains("pointer")?!0:!1;l=ia(e,"Node",l).append("table").attr("class","table table-condensed table-bordered").append("tbody");
if(g.name){var k=l.append("tr");k.append("td").text("Name");k.append("td").text(g.name)}if(g.taxonomies)for(var p=0;p<g.taxonomies.length;p++){var n=g.taxonomies[p],k=l.append("tr");k.append("td").text("Taxononomy");k=k.append("td");b.taxcolors[n.code]&&b.taxcolors[n.code].url&&(k=k.append("a").attr("href",b.taxcolors[n.code].url).attr("target","_blank"));var h="",h=h+(b.taxcolors[n.code]&&b.taxcolors[n.code].name?b.taxcolors[n.code].name+" ":""),h=h+" ",h=h+(n.scientificName?"\x3ci\x3e"+n.scientificName+
"\x3c/i\x3e":""),h=h+" ",h=h+(n.commonName?n.commonName:""),h=h+" ",h=h+(n.code?"("+n.code+")":"");k.html(h)}if(g.confidences)for(p=0;p<g.confidences.length;p++)k=l.append("tr"),k.append("td").text("Confidence"),k.append("td").html(g.confidences[p].value);k=l.append("tr");k.append("td").text("Depth");k.append("td").text(g.depth);k=l.append("tr");k.append("td").text("Branch length");k.append("td").text(g.branchLength?g.branchLength:0);k=l.append("tr");k.append("td").text("Distance from root");k.append("td").text(g.rootDist);
for(p=0;p<g.properties.length;p++)k=g.properties[p],t=k.ref,n=k.value,n=n.trim(),/^http:\/\//.test(n)&&(n="\x3ca href\x3d'"+n+"' target\x3d'_blank'\x3e"+n+"\x3c/a\x3e\x3cbr /\x3e"),k=l.append("tr"),k.append("td").text(t),k.append("td").html(n);for(var t in g.events)n=g.events[t],k=l.append("tr"),k.append("td").text(t),k.append("td").html(n);if(g.sequences)for(p=0;p<g.sequences.length;p++){t=g.sequences[p];n="";n+=t.symbol?t.symbol+" ":"";n+=t.name?t.name+" ":"";n+=t.location?t.location+" ":"";n+=
t.geneName?t.geneName:"";n.length&&(k=l.append("tr"),k.append("td").text("Sequence"),k.append("td").html(n));t.accession&&(n="",n+=t.accession.value?t.accession.value+" ":"",n+=t.accession.source?"["+t.accession.source+"] ":"",n+=t.accession.comment?"("+t.accession.comment+")":"",n.length&&(k=l.append("tr"),k.append("td").text("Accession"),k.append("td").html(n)));t.molSeq&&t.molSeq.value&&(k=l.append("tr"),k.append("td").text("Molecular Sequence"),k.append("td").html(t.molSeq.value));if(t.uris){n=
"";for(k=0;k<t.uris.length;k++)n=t.uris[k],n="\x3ca href\x3d'"+n.value+"'\x3e"+(n.desc?n.desc:n.value)+"\x3c/a\x3e";n.length&&(k=l.append("tr"),k.append("td").text("See also"),k.append("td").html(n))}if(g.sequences[p].domainArchitecture&&g.sequences[p].domainArchitecture.domains){l=f.target.parentNode.classList&&f.target.parentNode.classList.contains("domain")?!0:!1;l=ia(e,"Domains",l).append("table").attr("id","dt"+u).attr("class","table table-condensed table-bordered");t=l.append("thead").append("tr");
t.append("th").text("#");t.append("th").text("Name");t.append("th").text("Description");t.append("th").text("From");t.append("th").text("To");t.append("th").text("P value");h=l.append("tbody");for(n=0;n<g.sequences[p].domainArchitecture.domains.length;n++){var F=g.sequences[p].domainArchitecture.domains[n],k=h.append("tr");k.append("td").style("background",b.domcolors[F.name]&&b.domcolors[F.name].color?b.domcolors[F.name].color.replace("0x","#"):"").text(" ");t=k.append("td");b.domcolors[F.name]&&
b.domcolors[F.name].url&&(t=t.append("a").attr("href",b.domcolors[F.name].url).attr("target","_blank"));t.text(F.name);k.append("td").text(b.domcolors[F.name]?b.domcolors[F.name].description:"");k.append("td").text(F.from);k.append("td").text(F.to);k.append("td").text(F.confidence)}jQuery("#dt"+u).dataTable({searching:!1,paging:!1,language:{info:""},order:[[3,"asc"]],columns:[{orderable:!1},null,null,null,null,null]})}}if(b.graphs)for(p=0;p<b.graphs.length;p++)if(h=b.graphs[p],h.data&&h.data[g.id]&&
h.data[g.id].length)for(l=f.target.parentNode.classList&&f.target.parentNode.classList.contains(h.type)?!0:!1,l=ia(e,h.name,l).append("table").attr("class","table table-condensed table-bordered").append("tbody"),n=0;n<h.legend.fields.length;n++)k=l.append("tr"),t=k.append("td"),h.legend.fields[n].url&&(t=t.append("a").attr("href",h.legend.fields[n].url).attr("target","_blank")),t.text(h.legend.fields[n].name),k.append("td").text(h.data[g.id][n]);m.selectAll("g.node.cid_"+g.id).selectAll("rect.pointer").style("opacity",
"1");f.stopPropagation()}function oa(){a.lineupNodes?m.selectAll("g.leaf").append("path").attr("class","support").attr("stroke",z()).attr("stroke-dasharray","2,3").attr("stroke-width","0.5px"):m.selectAll("path.support").remove()}function X(g,d){var e="";if(a.showTaxonomy&&g.taxonomies)for(var f=0;f<g.taxonomies.length;f++){var l=g.taxonomies[f];a.showFullTaxonomy&&(e+=b.taxcolors[l.code]&&b.taxcolors[l.code].name?b.taxcolors[l.code].name:"",e+=" ",e+=l.scientificName?l.scientificName:"",e+=" ",e+=
l.commonName?l.commonName:"",e+=" ");e+=l.code?l.code:"";e+=" "}f="";if(a.showSequences&&g.sequences)for(l=0;l<g.sequences.length;l++){var k=g.sequences[l],f=f+(k.symbol?k.symbol:""),f=f+" ",f=f+(k.accession?k.accession.value:""),f=f+" ",f=f+(k.name?k.name:""),f=f+" ",f=f+(k.geneName?k.geneName:""),f=f+" ";if(k.annotations)for(var p=0;p<k.annotations.length;p++)text+=k.annotations[p].desc?k.annotations[p].desc:"",text+=" "}l=a.showNodeNames&&g.name?g.name+" ":" ";d||(g.children&&"only leaf"==a.showNodesType&&
(l=" "),g.children||"only inner"!=a.showNodesType||(l=" "));return e+l+f}function S(){var g=0;ja=0;B.selectAll("text.name").text(function(a){var b=X(a);b.length>g&&(g=b.length,ja=a.id);return b});m.selectAll("g.node.inner").selectAll("text.name").text(X);a.showGraphs&&T();a.showDomains&&K()}function J(){m.selectAll("text.branchLength").attr("visibility",function(a){return a.show?"visible":"hidden"});m.selectAll("text.supportValues").attr("visibility",function(a){return a.show?"visible":"hidden"});
m.selectAll("text.name").attr("visibility",function(a){return a.show?"visible":"hidden"});a.lineupNodes&&B.selectAll("path.support").attr("visibility",function(a){return a.show?"visible":"hidden"});a.showLabels&&ta();a.showGraphs&&ua();a.showDomains&&ba()}function Z(){m.selectAll("text.name").attr("stroke",z()).attr("stroke-width",a.outline+"px").attr("fill",function(g){var d=null;if(a.showTaxonomyColors&&b.taxcolors&&g.taxonomies)for(var e=0;e<g.taxonomies.length;e++){var f=g.taxonomies[e];b.taxcolors[f.code]&&
b.taxcolors[f.code].color&&(d=b.taxcolors[f.code].color.replace(/0x/,"#"))}return d?d:z()})}function va(){B.selectAll("text.name").attr("dx",function(b){return parseInt(a.lineupNodes?phyd3.phylogram.dx-b.y+5:5)});B.selectAll("path.support").attr("d",function(a){return"M0,0 L"+parseInt(phyd3.phylogram.dx-a.y)+",0"});a.showLabels&&ka();a.showGraphs&&T();a.showDomains&&K()}function E(){var b=0;m.selectAll("text.name").attr("dy",a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");m.selectAll("text.branchLength").attr("dy",
-a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");m.selectAll("text.supportValue").attr("dy",a.nodeHeight/2+5).attr("font-size",1.5*a.nodeHeight+"px");m.selectAll("g.leaf.cid_"+ja).selectAll("text.name").each(function(){var a=this.getBBox();a.width>b&&(b=a.width)});b+=10;b<a.textLength&&(b=a.textLength);O=b;d3.select("#nodeHeight").attr("value",2*a.nodeHeight+"px");va();a.showLabels&&ka();a.showGraphs&&T();a.showDomains&&K()}function pa(){if(a.showLabels&&b.labels){for(var g=0;g<b.labels.length;g++){var d=
b.labels[g];d.id||(d.id=parseInt(Date.now()*Math.random()*1E3));d.data.tag&&m.selectAll("g.node").each(function(a){var b=a[d.data.tag],g=d.data.ref,g=g?g:"value",b="object"==typeof b?b[g]:b;if("property"==d.data.tag)for(pid=0;pid<a.properties.length;pid++){var e=a.properties[pid];if(e.ref==g){b=e.value;break}}b&&(d.data[a.id]=b)});switch(d.type){case "text":for(cid in d.data)d.data[cid]&&m.selectAll(".cid_"+cid).append("text").attr("class","nodelabel text lid"+d.id).attr("dy",3).attr("text-anchor",
"start").attr("stroke-width",a.outline+"px").attr("stroke",z()).attr("fill",z()).attr("font-size",1.5*a.nodeHeight+"px").text(d.data[cid]).append("title").text(d.name+": "+d.data[cid]);break;case "color":for(cid in d.data)if(d.data[cid]){var e=a.nodeHeight,f=d.data[cid].replace(/0x/,"#");m.selectAll(".cid_"+cid).append("rect").attr("height",2*e).attr("width",2*e).attr("class","nodelabel color lid"+d.id).attr("style","fill: "+f+"; stroke: "+f).append("title").text(d.name)}}m.append("text").attr("class",
"labellegend lid"+d.id).attr("fill",z()).text(0!=d.showLegend?d.name:"")}ta();ka()}else m.selectAll(".nodelabel").remove(),m.selectAll(".labellegend").remove(),P=0;T();K()}function ka(){var d=a.nodeHeight;P=0;if(b.labels){for(var f=0;f<b.labels.length;f++){var e=b.labels[f],u=0;m.selectAll(".nodelabel.lid"+e.id).attr("transform",function(a){var b=0,g=0;"color"==e.type&&(b=g=-d);var f=0;"text"==this.nodeName&&this.firstChild?f=this.firstChild.length*d/1.5:"rect"==this.nodeName&&(f=2*d);f>u&&(u=f);
a.branchset.length?(b+=a.y,b-=d):b+=phyd3.phylogram.dx+O;return"translate("+parseInt(b+d+P-a.y)+","+g+")"});m.selectAll("text.labellegend.lid"+e.id).attr("transform","translate("+parseInt(phyd3.phylogram.dx+P+O+2*d)+",-10) rotate(-90)");P+=u+2*d}f=B.selectAll(".nodelabel");f.selectAll("text").attr("dy",a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");f.selectAll("rect").attr("width",2*a.nodeHeight).attr("height",2*a.nodeHeight)}}function ta(){m.selectAll(".nodelabel").attr("visibility",function(a){return a.show?
"visible":"hidden"})}function na(){if(a.showGraphs&&b.graphs&&b.graphs.length){d3.svg.arc().innerRadius(0).outerRadius(a.nodeHeight);for(var d=d3.layout.pie().value(function(a){return a.value}).sort(null),f=w=0;f<b.graphs.length;f++){var e=b.graphs[f];e.id||(e.id=parseInt(Date.now()*Math.random()*1E3));if(e.data.tag)var u=m.selectAll("g.node").each(function(a){var b=a[e.data.tag],d=e.data.ref,d=d?d:"value",b="object"==typeof b?b[d]:b;if("property"==e.data.tag)for(var g=0;g<a.properties.length;g++){var f=
a.properties[g];if(f.ref==d){b=f.value;break}}b&&(e.data[a.id]=[],e.data[a.id][0]=b,"pie"==e.type&&e.data.max&&(e.data[a.id][1]=e.data.max-b,e.legend.fields[1]={name:e.legend.fields[0].name,color:e.legend.fields[0].background?e.legend.fields[0].background:phyd3.phylogram.randomColor(),invert:!0}))});switch(e.type){case "pie":for(cid in e.data)if(u=cid.replace(/\./g,"\\."),e.data[cid]&&Array.isArray(e.data[cid])){var l=!1;m.select(".cid_"+u).each(function(a){0==a.branchset.length&&(l=!0)});for(var u=
m.selectAll(".cid_"+u).append("svg:g").attr("class","graph pie gid"+e.id),k=[],p=0;p<e.data[cid].length;p++)void 0!=e.data[cid][p]&&k.push({i:p,value:e.data[cid][p],leaf:l});k=u.selectAll("path.pie.gid"+e.id).data(d(k)).enter().append("path").attr("class","pie hover-visible gid"+e.id).attr("stroke",z()).attr("stroke-width",a.outline+"px").attr("fill",function(a,b){return e.legend.fields?(e.legend.fields[b]||(e.legend.fields[b]={},e.legend.fields[b].name="Series "+(b+1),e.legend.fields[b].color=phyd3.phylogram.randomColor()),
e.legend.fields[b].color.replace(/0x/,"#")):""}).append("title").text(function(a,b){return e.legend.fields?(e.legend.fields[b]&&e.legend.fields[b].name?e.legend.fields[b].name+": ":"")+(e.legend.fields[b].invert?e.data.max-a.data.value:a.data.value):""})}break;case "binary":for(cid in e.data)if(e.data[cid]&&Array.isArray(e.data[cid])){u=cid.replace(/\./g,"\\.");k=[];l=!1;m.select(".cid_"+u).each(function(a){0==a.branchset.length&&(l=!0)});u=m.selectAll(".cid_"+cid).append("svg:g").attr("class","graph binary gid"+
e.id);for(p=0;p<e.data[cid].length;p++)void 0!=e.data[cid][p]&&k.push({i:p,value:e.data[cid][p],shape:e.legend.fields[p]?e.legend.fields[p].shape:"",leaf:l});k=u.selectAll("path.binary.gid"+e.id).data(k).enter().append("path").attr("class","binary hover-visible gid"+e.id).attr("style",function(a,b){b=(e.legend.fields[b]?e.legend.fields[b].color:"").replace(/0x/,"#");return"fill:"+(1<=a.value?b:"none")+";stroke:"+(0<=a.value?b:"none")}).append("title").text(function(a,b){return(e.legend.fields[b]?
e.legend.fields[b].name+": ":"")+a.value})}break;case "multibar":C[e.id]=[];for(p=0;p<e.legend.fields.length;p++)k=d3.max(d3.values(e.data),function(a){if(a)return a[p]}),u=d3.min(d3.values(e.data),function(a){if(a)return a[p]}),0<u&&(u=0),C[e.id][p]=d3.scale.linear().domain([u,k]);for(cid in e.data)if(e.data[cid]){u=cid.replace(/\./g,"\\.");u=m.select(".cid_"+u).append("svg:g").attr("class","graph multibar gid"+e.id);k=[];for(p=0;p<e.data[cid].length;p++)void 0!=e.data[cid][p]&&k.push({i:p,value:e.data[cid][p]});
k=u.selectAll("rect.multibar.gid"+e.id).data(k).enter().append("rect").attr("class","multibar hover-visible gid"+e.id).attr("fill",function(a,b){return(e.legend.fields[b]?e.legend.fields[b].color:"").replace(/0x/,"#")}).attr("stroke",z()).attr("stroke-width",a.outline).append("title").text(function(a,b){return(e.legend.fields[b]?e.legend.fields[b].name+": ":"")+a.value})}break;case "boxplot":D[e.id]=null;for(var h=0,q=Number.MAX_SAFE_INTEGER,p=0;5>p;p++)k=d3.max(d3.values(e.data),function(a){if(a)return a[p]}),
u=d3.max(d3.values(e.data),function(a){if(a)return a[p]}),k>h&&(h=k),u<q&&(q=u);D[e.id]=d3.scale.linear().domain([q,h]);for(cid in e.data)e.data[cid]&&(u=cid.replace(/\./g,"\\."),u=m.select(".cid_"+u).append("svg:g").attr("class","graph boxplot gid"+e.id),k=[],k.push({min:e.data[cid][0],q1:e.data[cid][1],median:e.data[cid][2],q3:e.data[cid][3],max:e.data[cid][4]}),k=u.selectAll("rect.boxplot.gid"+e.id).data(k).enter(),k.append("line").attr("class","boxplot baseline hover-visible gid"+e.id).attr("style",
"marker-start: url(#markerBoxplot); marker-end: url(#markerBoxplot);").attr("stroke",z()),k.append("rect").attr("class","boxplot q1 hover-visible gid"+e.id).attr("fill",(e.legend.fields[0]?e.legend.fields[0].color:"").replace(/0x/,"#")).attr("stroke",z()).attr("stroke-width",a.outline).append("title").text(function(a){return"Min: "+a.min+", Q1: "+a.q1+", Median: "+a.median}),k.append("rect").attr("class","boxplot q3 hover-visible gid"+e.id).attr("fill",(e.legend.fields[1]?e.legend.fields[1].color:
"").replace(/0x/,"#")).attr("stroke",z()).attr("stroke-width",a.outline).append("title").text(function(a){return"Median: "+a.median+", Q3: "+a.q3+", Max: "+a.max}),k.append("line").attr("class","boxplot median hover-visible gid"+e.id).attr("stroke",z()).attr("stroke-width","2px"));break;case "heatmap":var k=d3.max(d3.values(e.data),function(a){if(a)return d3.max(a)}),u=d3.min(d3.values(e.data),function(a){if(a)return d3.min(a)}),t=d3.scale.quantize().domain([u,k]).range(colorbrewer[e.legend.gradient.name][e.legend.gradient.classes]);
for(cid in e.data)if(e.data[cid]){u=cid.replace(/\./g,"\\.");u=m.select(".cid_"+u).append("svg:g").attr("class","graph heatmap gid"+e.id);k=[];if(e.data[cid])for(p=0;p<e.data[cid].length;p++)void 0!=e.data[cid][p]&&k.push({i:p,value:e.data[cid][p]});k=u.selectAll("rect.heatmap.gid"+e.id).data(k).enter().append("rect").attr("class","heatmap hover-visible gid"+e.id).attr("fill",function(a,b){return t(a.value)}).attr("stroke",z()).attr("stroke-width",a.outline+"px").append("title").text(function(a,b){return(e.legend.fields[b]?
e.legend.fields[b].name+": ":"")+a.value})}break;default:console.log("Graph not supported "+e.type)}}ua();T()}else w=0,m.selectAll("g.graph").remove(),m.selectAll("text.legend").remove();K()}function T(){var d=a.nodeHeight,f=O+P;B.selectAll("g.graph").attr("transform",function(a){return"translate("+(phyd3.phylogram.dx-a.y+f)+",0)"});var e=m.selectAll("g.graph");d3.select("#graphWidth").attr("value",a.graphWidth);m.selectAll("text.legend").remove();w=0;if(b.graphs){for(var h=0;h<b.graphs.length;h++){var l=
b.graphs[h];switch(l.type){case "pie":e.selectAll("path.pie.gid"+l.id).attr("d",function(a){if(0<a.value)return d3.svg.arc().innerRadius(0).outerRadius(d)(a)}).attr("transform",function(a){return" translate("+parseInt(a.data.leaf?d+w:0)+",0)"});a.showGraphs&&a.showGraphLegend&&m.append("text").attr("class","legend").text(0!=l.legend.show?l.name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+f+w+2*d)+",-10) rotate(-90)");w+=2*d+5;break;case "binary":e.selectAll("path.binary.gid"+l.id).attr("d",
function(a){var b=d3.svg.symbol().size(4*(d-2)*(d-2));b.type(l.legend.fields[a.i].shape);return b(a)}).attr("transform",function(a){return" translate("+(a.leaf?w+10+a.i*(2*d+5):0)+" ,0)"});if(a.showGraphs&&a.showGraphLegend)for(var k=0;k<l.legend.fields.length;k++)m.append("text").attr("class","legend").text(0!=l.legend.show?l.legend.fields[k].name:"").attr("transform","translate("+(phyd3.phylogram.dx+f+w+k*(2*d+5)+2*d)+", -10) rotate(-90)");w+=(l.legend.fields.length+1)*(2*d+5)+5;break;case "multibar":for(k=
0;k<l.legend.fields.length;k++)C[l.id][k]=C[l.id][k].range([0,a.graphWidth]);e.selectAll("rect.multibar.gid"+l.id).attr("height",parseInt(2*d)).attr("width",function(a,b){b=parseInt(C[l.id]&&C[l.id][a.i]?C[l.id][a.i](a.value):0);a=parseInt(C[l.id]&&C[l.id][a.i]?C[l.id][a.i](0):0);return b>a?b-a:a-b}).attr("transform",function(b,e){e=w+b.i*(a.graphWidth+5);var g=parseInt(C[l.id]&&C[l.id][b.i]?C[l.id][b.i](b.value):0);b=parseInt(C[l.id]&&C[l.id][b.i]?C[l.id][b.i](0):0);return" translate("+parseInt(e+
(b<g?b:g))+",-"+parseInt(d)+")"}).attr("x",function(a,b){});if(a.showGraphs&&a.showGraphLegend)for(k=0;k<l.legend.fields.length;k++)m.append("text").attr("class","legend").text(0!=l.legend.show?l.legend.fields[k].name:"").attr("transform","translate("+(phyd3.phylogram.dx+f+w+k*(a.graphWidth+5)+a.graphWidth/2)+",-10) rotate(-90)");w+=l.legend.fields.length*(a.graphWidth+5);break;case "boxplot":D[l.id]=D[l.id].range([0,a.graphWidth]);e.selectAll("rect.q1.boxplot.gid"+l.id).attr("height",parseInt(2*
d)).attr("width",function(a){return parseInt(D[l.id](a.median)-D[l.id](a.q1))}).attr("transform",function(a,b){a=w+D[l.id](a.q1);return" translate("+parseInt(a)+",-"+parseInt(d)+")"});e.selectAll("rect.q3.boxplot.gid"+l.id).attr("height",parseInt(2*d)).attr("width",function(a){return parseInt(D[l.id](a.q3)-D[l.id](a.median))}).attr("transform",function(a,b){a=w+D[l.id](a.median);return" translate("+parseInt(a)+",-"+parseInt(d)+")"});e.selectAll("line.baseline.boxplot.gid"+l.id).attr("y1",parseInt(d)).attr("y2",
parseInt(d)).attr("x1",function(a){return D[l.id](a.min)}).attr("x2",function(a){return D[l.id](a.max)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(d)+")"});e.selectAll("line.median.boxplot.gid"+l.id).attr("y1",0).attr("y2",parseInt(2*d)).attr("x1",function(a){return D[l.id](a.median)}).attr("x2",function(a){return D[l.id](a.median)}).attr("transform",function(a,b){return" translate("+parseInt(w)+",-"+parseInt(d)+")"});a.showGraphs&&a.showGraphLegend&&m.append("text").attr("class",
"legend").text(0!=l.legend.show?l.name:"").attr("transform","translate("+(phyd3.phylogram.dx+f+w+2*d)+", -10) rotate(-90)");w+=a.graphWidth+5;break;case "heatmap":e.selectAll("rect.heatmap.gid"+l.id).attr("width",2*d).attr("height",2*d).attr("transform",function(a,b){return" translate("+(w+2*a.i*d)+",-"+d+")"});if(a.showGraphs&&a.showGraphLegend)for(k=0;k<l.legend.fields.length;k++)m.append("text").attr("class","legend").text(0!=l.legend.show?l.legend.fields[k].name:"").attr("transform","translate("+
parseInt(phyd3.phylogram.dx+f+w+(k+1)*d*2)+",-10) rotate(-90)");w+=2*l.legend.fields.length*d+5}}m.selectAll("text.legend").attr("fill",z()).attr("font-size",2*a.nodeHeight+"px")}}function Aa(){var d=a.nodeHeight;w=0;if(b.graphs)for(var f=0;f<b.graphs.length;f++){var e=b.graphs[f];switch(e.type){case "pie":w+=2*d+5;break;case "binary":w+=(e.legend.fields.length+1)*(2*d+5)+5;break;case "multibar":w+=e.legend.fields.length*(a.graphWidth+5);break;case "heatmap":w+=2*e.legend.fields.length*d+5}}return 2*
a.margin+(a.showNodeNames?O:0)+(a.showGraphs?w:0)+(a.showDomains?a.domainWidth:0)}function ua(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";for(var e=!0,g=a.nodeHeight,f=d.x,h=d.y,n=0;n<b.length;n++){var m=b[n].x,t=b[n].y;if((m-f)*(m-f)+(t-h)*(t-h)<=4*g*g){e=!1;break}}return e?(b.push(d),"visible"):"hidden"};B.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});m.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",d);m.selectAll("g.inner").selectAll(".graph.binary").attr("visibility",
d)}function M(d,h){phyd3.phylogram.lineAreas=[];phyd3.phylogram.dx=0;h=d3.layout.cluster().separation(function(a,b){return 1}).children(a.children||function(a){return a.branchset}).size([U*a.scaleY,L*a.scaleX]);I=h(b);a.showPhylogram||phyd3.phylogram.scaleBranchLengths(I,L*a.scaleX);var e=m.selectAll("path.link");d||(e=e.data(h.links(I)),e.enter().append("svg:path").attr("class","link").attr("fill","none").attr("stroke",function(a){if(a.source.groupIDs&&a.source.groupIDs.length){for(var d={depth:0},
e=0;e<a.source.groupIDs.length;e++){var f=b.groups[a.source.groupIDs[e]];f&&f.depth>=d.depth&&(d=f)}return d.foregroundColor}return z()}));e.attr("d",phyd3.phylogram.rightAngleDiagonal);V=void 0;c=0;e=I.filter(function(a){return!a.children});for(h=0;h<e.length;h++)e[h].show=a.dynamicHide?wa(e[h],!a.lineupNodes):!0;V=void 0;e=I.filter(function(a){return a.children});for(h=0;h<e.length;h++)e[h].show=a.dynamicHide?wa(e[h],!0):!0;d||(R=R.data(I),R.enter().append("svg:g").attr("class",function(a){return a.children?
0==a.depth?"root node cid_"+a.id:"inner node cid_"+a.id:"leaf node cid_"+a.id}).style("cursor","pointer").on("click",function(d){var e=d3.event;!e.ctrlKey||e.altKey||e.shiftKey?!e.altKey||e.ctrlKey||e.shiftKey?!e.shiftKey||e.altKey||e.ctrlKey?e.shiftKey||e.altKey||e.ctrlKey||(void 0!==a.popupAction?a.popupAction(d):za(d)):(jQuery("#groupID").val(d.id),jQuery("#groupDepth").val(d.depth),b.groups[d.id]?(jQuery("#groupLabel").val(b.groups[d.id].label),jQuery("#groupLabelForegroundCP").colorpicker("setValue",
b.groups[d.id].foregroundColor),jQuery("#groupLabelBackgroundCP").colorpicker("setValue",b.groups[d.id].backgroundColor)):(jQuery("#groupLabel").val(""),jQuery("#groupLabelForegroundCP").colorpicker("setValue",z()),jQuery("#groupLabelBackgroundCP").colorpicker("setValue",a.invertColors?a.foregroundColor:a.backgroundColor)),jQuery("#groupLabelModal").modal("show")):(xa(b,d.id),x(),A()):(ya(b,d.id),f(),A())}).style("z-index",100).append("rect").attr("class","pointer").attr("fill","red").attr("width",
2*a.nodeHeight+2+"px").attr("height",2*a.nodeHeight+2+"px").style("opacity","0").attr("x","-"+(a.nodeHeight+1)+"px").attr("y","-"+(a.nodeHeight+1)+"px"));R.attr("transform",function(a){return"translate("+parseInt(a.y)+","+parseInt(a.x)+")"});phyd3.phylogram.lineAreas=[]}function ra(b,d){if(X(b,!0).match(d)){var e=!b.children&&a.lineupNodes?phyd3.phylogram.dx-b.y:0;m.selectAll("g.node.cid_"+b.id).append("rect").attr("class","searchResults").attr("width",O+"px").attr("height",2*a.nodeHeight+2+"px").attr("fill",
"yellow").style("opacity","0.3").attr("x",e+"px").attr("y","-"+(a.nodeHeight+1)+"px")}if(b.children)for(e=0;e<b.children.length;e++)r=ra(b.children[e],d);return!1}function ya(a,b,d){if(d||a.id==b)d=!0,a.branchset=a.branchset.reverse();if(a.children)for(var e=0;e<a.children.length;e++)ya(a.children[e],b,d)}function xa(a,d){if(a.id==d)b.branchset=[a];else if(a.children)for(var e=0;e<a.children.length;e++)xa(a.children[e],d)}function Q(){M(!0);aa()}function wa(b,d){var e=!0,f=b.y+1,g=b.y+5.5*X(b,!0).length,
k=b.x-a.nodeHeight,h=b.x+a.nodeHeight;if(!V)return V=b,!0;a.lineupNodes&&!b.children&&(f+=phyd3.phylogram.dx-b.y,g+=phyd3.phylogram.dx-b.y);var n=V,m=n.y+1,t=n.y+5.5*X(n,!0).length,q=n.x-a.nodeHeight,v=n.x+a.nodeHeight;a.lineupNodes&&!n.children&&(m+=phyd3.phylogram.dx-n.y,t+=phyd3.phylogram.dx-n.y);if(m<=f&&f<=t&&q<=k&&k<=v||m<=g&&g<=t&&q<=k&&k<=v||m<=f&&f<=t&&q<=h&&h<=v||m<=g&&g<=t&&q<=h&&h<=v)e=!1;if(e&&d)for(d=0;d<phyd3.phylogram.lineAreas.length;d++)if(q=phyd3.phylogram.lineAreas[d],n=q.start[0],
m=q.end[0],t=q.start[1],q=q.end[1],t==q){if(k<=t&&q<=h&&!(n>g||m<f)){e=!1;break}}else if(n==m&&f<=n&&m<=g&&!(t>h||q<k)){e=!1;break}e&&(V=b);return e}function aa(){var b=ca.translate(),d=b[0]+a.marginX-a.translateX,e=b[1]+a.marginY-a.translateY;m.attr("transform",function(a){return"translate("+parseInt(d)+","+parseInt(e)+") scale(1)"});0!=a.lastScale&&(va(),a.dynamicHide&&J());qa();f()}a=a||{};a.scaleY=a.scaleY||1;a.scaleX=a.scaleX||1;a.translateX=a.translateX||0;a.translateY=a.translateY||0;a.height=
...
a.maxDecimalsLengthValues||2;a.foregroundColor=a.foregroundColor||"#000";a.backgroundColor=a.backgroundColor||"#fff";a.branchLengthColor=a.branchLengthColor||"red";a.supportValuesColor=a.supportValuesColor||"blue";a.showNodesType=a.showNodesType||"only leaf";a.treeWidth=a.treeWidth||"auto";a.showFullTaxonomy="showFullTaxonomy"in a?a.showFullTaxonomy:!1;a.showLabels="showLabels"in a?a.showLabels:!0;a.showDomains="showDomains"in a?a.showDomains:!0;a.dynamicHide="dynamicHide"in a?a.dynamicHide:!1;a.invertColors=
"invertColors"in a?a.invertColors:!1;a.lineupNodes="lineupNodes"in a?a.lineupNodes:!0;a.showSupportValues="showSupportValues"in a?a.showSupportValues:!1;a.showLengthValues="showLengthValues"in a?a.showLengthValues:!1;a.showTaxonomy="showTaxonomy"in a?a.showTaxonomy:!0;a.showTaxonomyColors="showTaxonomyColors"in a?a.showTaxonomyColors:!0;a.showDomainNames="showDomainNames"in a?a.showDomainNames:!1;a.showDomainColors="showDomainColors"in a?a.showDomainColors:!0;a.showGraphs="showGraps"in a?a.showGraphs:
!0;a.showGraphLegend="showGraphLegend"in a?a.showGraphLegend:!0;a.showNodeNames="showNodeNames"in a?a.showNodeNames:!0;a.showPhylogram="showPhylogram"in a?a.showPhylogram:!1;b.groups=b.groups?b.groups:{};var m,R,B,G,I,N,V,D=[],C=[],P=100,O=100,w=0,W=100,ja=0,Ba=jQuery.extend(!0,{},b),H=!1;if(b.graphs)for(var la=0;la<b.graphs.length;la++)if(0!=b.graphs[la].legend.show){H=!0;break}W=H?W:0;a.marginX=a.margin;a.marginY=a.margin+(a.showGraphs?W:0);a.initialGraphWidth=a.graphWidth;var Y=jQuery(d).width(),
L="auto"==a.treeWidth?Y-Aa():a.treeWidth,U=a.height-(2*a.margin+(a.showGraphs?W:0)),L=parseInt(L),U=parseInt(U);0>L&&(L=Y-2*O-(a.showGraphs?2*W:0));var ca=d3.behavior.zoom().scaleExtent([1,1]).on("zoom",function(b){b=0;var d=d3.event.sourceEvent;if(d){if(0<d.deltaY||0>d.wheelDelta)b=-1*a.scaleStep;if(0>d.deltaY||0<d.wheelDelta)b=a.scaleStep;var e=.5,f=.5;d.altKey||d.ctrlKey||(e=d.layerX/Y,f=d.layerY/a.height);if(d.ctrlKey||!d.altKey)a.scaleY+=b,a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY+=
U*b*f;if(d.altKey||!d.ctrlKey)a.graphWidth+=0==b?0:0>b?-10:10,10>a.graphWidth&&(a.graphWidth=10),a.domainWidth+=0==b?0:0>b?-1*a.domainWidthStep:a.domainWidthStep,a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep),a.scaleX+=b,a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX+=L*b*e;a.lastScale=b;0!=a.lastScale?requestAnimFrame(Q):requestAnimFrame(aa)}});d3.select("#dynamicHide").on("click",function(){a.dynamicHide=!a.dynamicHide;M(!0);J()});d3.select("#invertColors").on("click",
function(){a.invertColors=!a.invertColors;ga()});jQuery("#foregroundColorButton").colorpicker().on("changeColor",function(){a.foregroundColor=jQuery("#foregroundColor").val();ga()});jQuery("#backgroundColorButton").colorpicker().on("changeColor",function(){a.backgroundColor=jQuery("#backgroundColor").val();ga()});d3.select("#nodesType").on("change",function(){a.showNodesType=jQuery(this).val();Q();S()});d3.select("#phylogram").on("click",function(){a.showPhylogram=!a.showPhylogram;M(!0);E();J()});
d3.select("#lineupNodes").on("click",function(){a.lineupNodes=!a.lineupNodes;M(!0);oa();E();J()});d3.select("#lengthValues").on("click",function(){a.showLengthValues=!a.showLengthValues;a.showLengthValues?d3.select("#maxDecimalsLengthValues").attr("disabled",null):d3.select("#maxDecimalsLengthValues").attr("disabled","disabled");fa()});d3.select("#maxDecimalsLengthValues").on("change",function(){a.maxDecimalsLengthValues=jQuery(this).val();fa()});d3.select("#supportValues").on("click",function(){a.showSupportValues=
!a.showSupportValues;a.showSupportValues?d3.select("#maxDecimalsSupportValues").attr("disabled",null):d3.select("#maxDecimalsSupportValues").attr("disabled","disabled");ea()});d3.select("#maxDecimalsSupportValues").on("change",function(){a.maxDecimalsSupportValues=jQuery(this).val();ea()});d3.select("#nodeNames").on("click",function(){a.showNodeNames=!a.showNodeNames;S();E()});d3.select("#nodeLabels").on("click",function(){a.showLabels=!a.showLabels;pa()});d3.select("#taxonomy").on("click",function(){a.showTaxonomy=
...
function(){a.nodeHeight+=a.nodeHeightStep;M(!0);E();J();f()});d3.select("#resetZoom").on("click",function(){b=jQuery.extend(!0,{},Ba);x();A()});d3.select("#zoominY").on("click",function(){a.scaleY+=a.scaleStep;a.translateY+=U*a.scaleStep/2;a.lastScale=a.scaleStep;requestAnimFrame(Q)});d3.select("#zoomoutY").on("click",function(){a.scaleY-=a.scaleStep;a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY-=U*a.scaleStep/2;a.lastScale=-1*a.scaleStep;requestAnimFrame(Q)});d3.select("#zoominX").on("click",
...
null),d3.select("#graphWidthLower").attr("disabled",null),d3.select("#graphWidthHigher").attr("disabled",null)):(d3.select("#graphLegend").attr("disabled","disabled"),d3.select("#graphWidthLower").attr("disabled","disabled"),d3.select("#graphWidthHigher").attr("disabled","disabled"));na()});d3.select("#graphWidthLower").on("click",function(){a.graphWidth-=a.graphWidthStep;a.graphWidth<a.graphWidthStep&&(a.graphWidth=a.graphWidthStep);E()});d3.select("#graphWidthHigher").on("click",function(){a.graphWidth+=
a.graphWidthStep;E()});d3.select("#linkSVG").on("click",function(){var a=sa();saveAs(new Blob([unescape(encodeURIComponent(a))],{type:"application/svg+xml"}),"phylogram.svg")});d3.select("#linkPNG").on("click",function(){var a=sa(),b=document.createElement("canvas");d3.select("g#main").node().getBBox();canvg(b,a);b.toBlob(function(a){saveAs(a,"phylogram.png")})});d3.select("#searchQuery").on("change",function(){qa()});d3.select("#applyTaxonomyColors").on("click",function(){d3.selectAll("input.taxColor").each(function(a){b.taxcolors[a.taxonomy].color=
jQuery(this).val()});jQuery("#taxonomyColorsModal").modal("hide");Z()});d3.select("#applyGroupLabel").on("click",function(){var a=jQuery("#groupID").val(),d=jQuery("#groupDepth").val(),e=jQuery("#groupLabel").val(),m=jQuery("#groupLabelBackground").val(),l=jQuery("#groupLabelForeground").val();jQuery("#groupLabelModal").modal("hide");b.groups[a]={id:a,depth:d,label:e,foregroundColor:l,backgroundColor:m};h(b,a);A();f()});d3.select("#clearGroupLabel").on("click",function(){var a=jQuery("#groupID").val();
...
{show:parseInt(d.getAttribute("show")),fields:[],gradient:{}},a=d.getElementsByTagName("field"),f=0;f<a.length;f++){var h=a[f],v=h.getElementsByTagName("name")[0],v=v?v.textContent:"",q=h.getElementsByTagName("color")[0],q=q?q.textContent:"",x=h.getElementsByTagName("background")[0],x=x?x.textContent:"",A=h.getElementsByTagName("shape")[0],A=A?A.textContent:"",h=(h=h.getElementsByTagName("url")[0])?h.textContent:"";b.fields.push({name:v,color:q,background:x,shape:A,url:h})}if(d=d.getElementsByTagName("gradient")[0])v=
(v=d.getElementsByTagName("name")[0])?v.textContent:"",d=(d=d.getElementsByTagName("classes")[0])?parseInt(d.textContent):0,b.gradient={name:v,classes:d};return b};phyd3.phyloxml.parseData=function(d){var b={tag:d.getAttribute("tag"),ref:d.getAttribute("ref")},a=d.getElementsByTagName("min")[0];b.min=a?a.textContent:void 0;a=d.getElementsByTagName("max")[0];b.max=a?a.textContent:void 0;d=d.getElementsByTagName("values");for(a=0;a<d.length;a++){var f=d[a].getAttribute("for"),h=d[a].getElementsByTagName("value");
b[f]=[];for(var v=0;v<h.length;v++)b[f].push(parseFloat(h[v].textContent))}return b};phyd3.phyloxml.parseGraph=function(d){for(var b={name:"",legend:{},data:{},type:d.getAttribute("type")},a=0;a<d.childNodes.length;a++){var f=d.childNodes[a];switch(f.nodeName){case "name":b.name=f.textContent;break;case "legend":b.legend=phyd3.phyloxml.parseLegend(f);break;case "data":b.data=phyd3.phyloxml.parseData(f);break;case "#text":case "#comment":break;default:console.log("Undefined tag: "+f.nodeName+" "+f.textContent+
...
h.textContent;break;case "description":b.description=h.textContent;break;case "id":b.id=h.textContent;break;case "name":b.name=h.textContent;break;case "property":b.properties.push(phyd3.phyloxml.parseProperty(h));break;case "#text":case "#comment":break;default:console.log("Undefined tag: "+h.nodeName+" "+h.textContent+" - skipping...")}}var a=d.getElementsByTagName("taxonomies"),a=a[0],v={};if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++){var q=a.childNodes[f];if("color"==q.nodeName){var h=
q.getElementsByTagName("code")[0].textContent,x=q.getElementsByTagName("value")[0].textContent;v[h]={color:x}}if("taxonomy"==q.nodeName){var h=q.getAttribute("code"),x=(x=q.getElementsByTagName("color")[0])?x.textContent:x,A=q.getElementsByTagName("name")[0],A=A?A.textContent:A,q=(q=q.getElementsByTagName("url")[0])?q.textContent:q;v[h]={color:x,name:A,url:q}}}b.taxcolors=v;a=d.getElementsByTagName("domains");a=a[0];v={};if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++)q=a.childNodes[f],"color"==
q.nodeName&&(h=q.getElementsByTagName("code")[0].textContent,x=q.getElementsByTagName("value")[0].textContent,v[h]={color:x}),"domain"==q.nodeName&&(h=q.getAttribute("name"),x=(x=q.getElementsByTagName("color")[0])?x.textContent:x,A=(A=q.getElementsByTagName("description")[0])?A.textContent:A,q=(q=q.getElementsByTagName("url")[0])?q.textContent:q,v[h]={color:x,description:A,url:q});b.domcolors=v;a=d.getElementsByTagName("graphs");a=a[0];h=[];if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++)x=
a.childNodes[f],"graph"==x.nodeName&&h.push(phyd3.phyloxml.parseGraph(x));b.graphs=h;a=d.getElementsByTagName("labels");a=a[0];d=[];if(a&&a.childNodes)for(f=0;f<a.childNodes.length;f++)h=a.childNodes[f],"label"==h.nodeName&&d.push(phyd3.phyloxml.parseLabel(h));b.labels=d;return b}})();
/* Blob.js
