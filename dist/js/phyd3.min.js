...
l=T.value,g=n.value;e.value="translate("+a.marginX+", "+a.marginY+")";T.value=(Math.max(+T.value.replace("px",""),f.width)+a.marginX).toString();n.value=(Math.max(+n.value.replace("px",""),f.height)+a.marginY).toString();if("undefined"!=typeof window.XMLSerializer)var x=(new XMLSerializer).serializeToString(d);else"undefined"!=typeof d.xml&&(x=d.xml);e.value=k;T.value=l;n.value=g;return x}function m(){a.showDomains?(F=phyd3.phylogram.scaledomainWidths(A,a.domainWidth),u.append("svg:path").attr("class",
"domain").attr("stroke",e()).attr("visibility",function(a,d){return a.show?"visibile":"hidden"}),B=B.data(function(a,d,b){a=a.sequences&&a.sequences[0]&&a.sequences[0].domainArchitecture?a.sequences[0].domainArchitecture.domains:[];for(b=0;b<a.length;b++)a[b].i=d;a.sort(function(a,d){return Math.abs(d.to-d.from)-Math.abs(a.to-a.from)});return a}).enter().append("svg:g").attr("class","domain"),B.append("svg:rect").attr("class","domain hover-visible").attr("stroke",a.foregroundColor).attr("stroke-width",
a.outline+"px").append("title").text(function(a){return a.name}),t(),C(),w()):(B.remove(),u.selectAll("path.domain").remove(),B=u.selectAll("g.domain"))}function h(){a.showDomainNames?(B.append("svg:text").attr("class","domain").attr("dy",-3).attr("fill",e()).attr("font-size","10px").text(function(a){return a.name}),C()):B.selectAll("text.domain").remove()}function t(){var d=u.data();u.selectAll("g.domain").attr("visibility",function(b,f,e){return d[e].show&&b.confidence<=a.domainLevel?"visible":
"hidden"});u.selectAll("path.domain").attr("visibility",function(a){return a.show?"visible":"hidden"});d3.select("#domainLevel").attr("value",a.domainLevel.toPrecision(1))}function w(){B.selectAll("rect.domain").attr("fill",function(b){return a.showDomainColors&&d.domcolors[b.name]?d.domcolors[b.name].color.replace(/0x/,"#"):e()})}function C(){var d=u.data(),b=H+(a.showLabels?I:0)+(a.showGraphs?v:0);F=phyd3.phylogram.scaledomainWidths(A,a.domainWidth);u.selectAll("path.domain").attr("d",function(f,
e,q){e=a.lineupNodes?phyd3.phylogram.dx-d[q].y:0;f=f.sequences&&f.sequences[0]&&f.sequences[0].domainArchitecture?f.sequences[0].domainArchitecture.sequenceLength:0;return"M"+parseInt(b+e)+",0L"+parseInt(b+e+F(f))+",0"});B.selectAll("rect.domain").attr("width",function(a){return parseInt(F(a.to-a.from))}).attr("transform",function(f,e,q){return"translate("+parseInt(b+(a.lineupNodes?phyd3.phylogram.dx-d[f.i].y:0)+F(f.from))+","+parseInt(-1*a.nodeHeight)+")"}).attr("height",2*a.nodeHeight-2);B.selectAll("text.domain").attr("transform",
function(f){return"translate("+parseInt(b+(a.lineupNodes?phyd3.phylogram.dx-d[f.i].y:0)+F(f.from))+","+parseInt(-1*a.nodeHeight)+")"});d3.select("#domainWidth").attr("value",a.domainWidth)}function aa(){a.showSupportValues?(p.selectAll("g.inner.node").append("svg:text").attr("class","supportValue").attr("dx",-2).attr("text-anchor","end").attr("fill",a.supportValuesColor).text(function(a){var d="";if(a.confidences)for(var b in a.confidences)d+=parseFloat(a.confidences[b].value)+" ";return d}),z()):
p.selectAll("text.supportValue").remove()}function ba(){a.showLengthValues?(p.selectAll("g.node").append("svg:text").attr("class","branchLength").attr("dx",-2).attr("text-anchor","end").attr("fill",a.branchLengthColor).text(function(a){return a.branchLength?parseFloat(a.branchLength).toFixed(5):""}),z()):p.selectAll("text.branchLength").remove()}function V(a,d,b){var f=a.attr("id")+"_"+a.selectAll("div.panel-default").size(),e=a.append("div").attr("class","panel panel-default");e.append("div").attr("class",
"panel-heading").attr("role","tab").attr("id","heading"+f).append("h4").attr("class","panel-title").append("a").attr("class","collapsed").attr("aria-expanded",b?"true":"false").attr("role","button").attr("data-toggle","collapse").attr("data-parent","#"+a.attr("id")).attr("href","#collapse"+f).attr("aria-controls","collapse"+f).html(d);return e.append("div").attr("id","collapse"+f).attr("class","panel-collapse collapse "+(b?"in":"")).attr("role","tabpanel").attr("aria-labelledby","heading"+f).append("div").attr("class",
"panel-body table-responsive")}function ka(e){var r=d3.event;if(!r.defaultPrevented){var f=r.layerX;y=r.layerY;var q="popup"+parseInt(Date.now()*Math.random()*1E3);console.log(r);var f=d3.select(b).append("div").attr("class","popup").attr("id",q).style("position","absolute").style("top",parseInt(y)+"px").style("left",parseInt(f)+"px").style("width",parseInt(a.popupWidth)+"px").style("color",a.foregroundColor).style("background-color",a.backgroundColor),n=f.append("button").attr("class","btn btn-link").style("position",
"absolute").style("right","8px");n.append("span").attr("class","glyphicon glyphicon-remove").attr("aria-hidden","true");n.on("click",function(){d3.select("#"+q).remove();p.selectAll("g.node.cid_"+e.id).selectAll("rect.pointer").style("opacity","0")});f=f.append("div").attr("class","panel-group").attr("id","accordion_"+parseInt(Date.now()*Math.random()*1E3)).attr("role","tablist").attr("aria-multiselectable","true");n="text"==r.target.nodeName||r.target.classList.contains("pointer")?!0:!1;n=V(f,"Node",
n).append("table").attr("class","table table-condensed table-bordered").append("tbody");if(e.name){var k=n.append("tr");k.append("td").text("Name");k.append("td").text(e.name)}if(e.taxonomies)for(var l in e.taxonomies){var g=e.taxonomies[l],k=n.append("tr");k.append("td").text("Taxononomy");k=k.append("td");d.taxcolors[g.code]&&d.taxcolors[g.code].url&&(k=k.append("a").attr("href",d.taxcolors[g.code].url).attr("target","_blank"));var x="",x=x+(d.taxcolors[g.code]&&d.taxcolors[g.code].name?d.taxcolors[g.code].name+
" ":""),x=x+" ",x=x+(g.scientificName?"\x3ci\x3e"+g.scientificName+"\x3c/i\x3e":""),x=x+" ",x=x+(g.commonName?g.commonName:""),x=x+" ",x=x+(g.code?"("+g.code+")":"");k.html(x)}if(e.confidences)for(var la in e.confidences)k=n.append("tr"),k.append("td").text("Confidence"),k.append("td").html(e.confidences[la].value);k=n.append("tr");k.append("td").text("Depth");k.append("td").text(e.depth);k=n.append("tr");k.append("td").text("Branch length");k.append("td").text(e.branchLength?e.branchLength:0);k=
n.append("tr");k.append("td").text("Distance from root");k.append("td").text(e.rootDist);for(var h in e.properties)k=e.properties[h],m=k.ref,l=k.value,l=l.trim(),/^http:\/\//.test(l)&&(l="\x3ca href\x3d'"+l+"' target\x3d'_blank'\x3e"+l+"\x3c/a\x3e\x3cbr /\x3e"),k=n.append("tr"),k.append("td").text(m),k.append("td").html(l);for(var m in e.events)l=e.events[m],k=n.append("tr"),k.append("td").text(m),k.append("td").html(l);if(e.sequences)for(var t in e.sequences)if(e.sequences[t].domainArchitecture&&
e.sequences[t].domainArchitecture.domains){n=r.target.parentNode.classList.contains("domain")?!0:!1;n=V(f,"Domains",n).append("table").attr("id","dt"+q).attr("class","table table-condensed table-bordered");m=n.append("thead").append("tr");m.append("th").text("#");m.append("th").text("Name");m.append("th").text("Description");m.append("th").text("From");m.append("th").text("To");m.append("th").text("P value");n=n.append("tbody");for(h=0;h<e.sequences[t].domainArchitecture.domains.length;h++)l=e.sequences[t].domainArchitecture.domains[h],
...
paging:!1,language:{info:""},order:[[3,"asc"]],columns:[{orderable:!1},null,null,null,null,null]})}if(d.graphs)for(t=0;t<d.graphs.length;t++)if(l=d.graphs[t],l.data&&l.data[e.id]&&l.data[e.id].length)for(n=r.target.parentNode.classList.contains(l.type)?!0:!1,n=V(f,l.name,n).append("table").attr("class","table table-condensed table-bordered").append("tbody"),h=0;h<l.legend.fields.length;h++)k=n.append("tr"),m=k.append("td"),l.legend.fields[h].url&&(m=m.append("a").attr("href",l.legend.fields[h].url).attr("target",
"_blank")),m.text(l.legend.fields[h].name),k.append("td").text(l.data[e.id][h]);p.selectAll("g.node.cid_"+e.id).selectAll("rect.pointer").style("opacity","1");r.stopPropagation()}}function ca(){a.lineupNodes?p.selectAll("g.leaf").append("path").attr("class","support").attr("stroke",e()).attr("stroke-dasharray","2,3").attr("stroke-width","0.5px"):p.selectAll("path.support").remove()}function P(b){if(b.children&&"only leaf"==a.showNodesType||!b.children&&"only inner"==a.showNodesType)return"";var e=
"";if(a.showTaxonomy&&b.taxonomies)for(var f in b.taxonomies){var q=b.taxonomies[f];a.showFullTaxonomy&&(e+=d.taxcolors[q.code]&&d.taxcolors[q.code].name?d.taxcolors[q.code].name:"",e+=" ",e+=q.scientificName?q.scientificName:"",e+=" ",e+=q.commonName?q.commonName:"",e+=" ");e+=q.code?q.code:"";e+=" "}e+=(a.showNodeNames&&b.name?b.name:"")+" ";if(a.showSequences&&b.sequences)for(var n in b.sequences)if(f=b.sequences[n],e+=f.symbol?f.symbol:"",e+=" ",e+=f.accession?f.accession.value:"",e+=" ",e+=f.name?
f.name:"",e+=" ",e+=f.geneName?f.geneName:"",e+=" ",f.annotations)for(var k in f.annotations)e+=f.annotations[k].desc?f.annotations[k].desc:"",e+=" ";return e}function Q(){var d=0;u.selectAll("text.name").text(function(a){a=P(a);a.length>d&&(d=a.length);return a});p.selectAll("g.node.inner").selectAll("text.name").text(P);a.showGraphs&&J();a.showDomains&&C()}function G(){u.selectAll("text.name").attr("visibility",function(a){return a.show?"visible":"hidden"});a.lineupNodes&&u.selectAll("path.support").attr("visibility",
function(a){return a.show?"visible":"hidden"});a.showLabels&&da();a.showGraphs&&ea();a.showDomains&&t()}function W(){p.selectAll("text.name").attr("stroke",e()).attr("stroke-width",a.outline+"px").attr("fill",function(b){var q=null;if(a.showTaxonomyColors&&d.taxcolors&&b.taxonomies)for(var f in b.taxonomies){var g=b.taxonomies[f];d.taxcolors[g.code]&&d.taxcolors[g.code].color&&(q=d.taxcolors[g.code].color.replace(/0x/,"#"))}return q?q:e()})}function fa(){u.selectAll("text.name").attr("dx",function(d){return parseInt(a.lineupNodes?
phyd3.phylogram.dx-d.y+5:5)});u.selectAll("path.support").attr("d",function(a){return"M0,0 L"+parseInt(phyd3.phylogram.dx-a.y)+",0"});a.showLabels&&X();a.showGraphs&&J();a.showDomains&&C()}function z(){p.selectAll("text.name").attr("dy",a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");p.selectAll("text.branchLength").attr("dy",-a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");p.selectAll("text.supportValue").attr("dy",a.nodeHeight/2+5).attr("font-size",1.5*a.nodeHeight+"px");H=100*a.nodeHeight/
6+10;d3.select("#nodeHeight").attr("value",a.nodeHeight);fa();a.showLabels&&X();a.showGraphs&&J();a.showDomains&&C()}function ga(){if(a.showLabels&&d.labels){console.log(d.labels);for(var b=0;b<d.labels.length;b++){var r=d.labels[b];r.id||(r.id=parseInt(Date.now()*Math.random()*1E3));r.data.tag&&p.selectAll("g.node").each(function(a){var b=a[r.data.tag],d=r.data.ref,d=d?d:"value",b="object"==typeof b?b[d]:b;if("property"==r.data.tag)for(pid in a.properties){var e=a.properties[pid];if(e.ref==d){b=
e.value;break}}b&&(r.data[a.id]=b)});switch(r.type){case "text":for(cid in r.data)r.data[cid]&&p.selectAll(".cid_"+cid).append("text").attr("class","nodelabel text lid"+r.id).attr("dy",3).attr("text-anchor","start").attr("stroke-width",a.outline+"px").attr("stroke",e()).attr("fill",e()).attr("font-size",1.5*a.nodeHeight+"px").attr("visibility","visible").text(r.data[cid]).append("title").text(r.name+": "+r.data[cid]);break;case "color":for(cid in r.data)if(r.data[cid]){var f=a.nodeHeight,g=r.data[cid].replace(/0x/,
"#");p.selectAll(".cid_"+cid).append("rect").attr("height",2*f).attr("width",2*f).attr("class","nodelabel color lid"+r.id).attr("style","fill: "+g+"; stroke: "+g).append("title").text(r.name)}}}da();X()}else p.selectAll(".nodelabel").remove(),I=0;J();C()}function X(){var b=a.nodeHeight;I=0;if(d.labels){for(var e=0;e<d.labels.length;e++){var f=d.labels[e],g=0;u.selectAll(".nodelabel.lid"+f.id).attr("transform",function(d){var e=0,l=0;"color"==f.type&&(e=l=-b);var q=0;"text"==this.nodeName&&this.firstChild?
q=this.firstChild.length*b/1.5:"rect"==this.nodeName&&(q=2*b);q>g&&(g=q);return"translate("+parseInt(e+b+I+(a.lineupNodes?phyd3.phylogram.dx-d.y+H:H))+","+l+")"});I+=g+2*b}e=u.selectAll(".nodelabel");e.selectAll("text").attr("dy",a.nodeHeight/2).attr("font-size",1.5*a.nodeHeight+"px");e.selectAll("rect").attr("width",2*a.nodeHeight).attr("height",2*a.nodeHeight)}}function da(){u.selectAll(".nodelabel").attr("visibility",function(a){return a.show?"visible":"hidden"})}function ha(){if(a.showGraphs&&
d.graphs&&d.graphs.length){d3.svg.arc().innerRadius(0).outerRadius(a.nodeHeight);for(var b=d3.layout.pie().value(function(a){return a.value}).sort(null),g=v=0;g<d.graphs.length;g++){var f=d.graphs[g];f.id||(f.id=parseInt(Date.now()*Math.random()*1E3));if(f.data.tag)var h=p.selectAll("g.node").each(function(a){var b=a[f.data.tag],d=f.data.ref,d=d?d:"value",b="object"==typeof b?b[d]:b;if("property"==f.data.tag)for(pid in a.properties){var e=a.properties[pid];if(e.ref==d){b=e.value;break}}b&&(f.data[a.id]=
[],f.data[a.id][0]=b,"pie"==f.type&&f.data.max&&(f.data[a.id][1]=f.data.max-b,f.legend.fields[1]={name:f.legend.fields[0].name,color:f.legend.fields[0].background?f.legend.fields[0].background:phyd3.phylogram.randomColor(),invert:!0}))});switch(f.type){case "pie":for(cid in f.data)if(f.data[cid]&&Array.isArray(f.data[cid])){var n=!1;p.select(".cid_"+cid).each(function(a){0==a.branchset.length&&(n=!0)});for(var h=p.selectAll(".cid_"+cid).append("svg:g").attr("class","graph pie gid"+f.id),k=[],l=0;l<
f.data[cid].length;l++)void 0!=f.data[cid][l]&&k.push({i:l,value:f.data[cid][l],leaf:n});h.selectAll("path.pie.gid"+f.id).data(b(k)).enter().append("path").attr("class","pie hover-visible gid"+f.id).attr("stroke",e()).attr("stroke-width",a.outline+"px").attr("fill",function(a,b){return f.legend.fields?(f.legend.fields[b]||(f.legend.fields[b]={},f.legend.fields[b].name="Series "+(b+1),f.legend.fields[b].color=phyd3.phylogram.randomColor()),f.legend.fields[b].color.replace(/0x/,"#")):""}).append("title").text(function(a,
b){return f.legend.fields?(f.legend.fields[b]&&f.legend.fields[b].name?f.legend.fields[b].name+": ":"")+(f.legend.fields[b].invert?f.data.max-a.data.value:a.data.value):""})}break;case "binary":for(cid in f.data)if(f.data[cid]&&Array.isArray(f.data[cid])){k=[];n=!1;p.select(".cid_"+cid).each(function(a){0==a.branchset.length&&(n=!0)});h=p.selectAll(".cid_"+cid).append("svg:g").attr("class","graph binary gid"+f.id);for(l=0;l<f.data[cid].length;l++)void 0!=f.data[cid][l]&&k.push({i:l,value:f.data[cid][l],
shape:f.legend.fields[l]?f.legend.fields[l].shape:"",leaf:n});h.selectAll("path.binary.gid"+f.id).data(k).enter().append("path").attr("class","binary hover-visible gid"+f.id).attr("style",function(a,b){b=(f.legend.fields[b]?f.legend.fields[b].color:"").replace(/0x/,"#");return"fill:"+(1<=a.value?b:"none")+";stroke:"+(0<=a.value?b:"none")}).append("title").text(function(a,b){return(f.legend.fields[b]?f.legend.fields[b].name+": ":"")+a.value})}break;case "multibar":K[f.id]=[];for(l=0;l<f.legend.fields.length;l++)h=
d3.max(d3.values(f.data),function(a){if(a)return a[l]}),K[f.id][l]=d3.scale.linear().domain([0,h]);for(cid in f.data)if(f.data[cid]){h=p.select(".cid_"+cid).append("svg:g").attr("class","graph multibar gid"+f.id);k=[];for(l=0;l<f.data[cid].length;l++)void 0!=f.data[cid][l]&&k.push({i:l,value:f.data[cid][l]});h.selectAll("rect.multibar.gid"+f.id).data(k).enter().append("rect").attr("class","multibar hover-visible gid"+f.id).attr("fill",function(a,b){return(f.legend.fields[b]?f.legend.fields[b].color:
"").replace(/0x/,"#")}).attr("stroke",a.foregroundColor).attr("stroke-width",a.outline).append("title").text(function(a,b){return(f.legend.fields[b]?f.legend.fields[b].name+": ":"")+a.value})}break;case "heatmap":var h=d3.max(d3.values(f.data),function(a){if(a)return d3.max(a)}),k=d3.min(d3.values(f.data),function(a){if(a)return d3.min(a)}),m=d3.scale.quantize().domain([k,h]).range(colorbrewer[f.legend.gradient.name][f.legend.gradient.classes]);for(cid in f.data)if(f.data[cid]){h=p.select(".cid_"+
cid).append("svg:g").attr("class","graph heatmap gid"+f.id);k=[];if(f.data[cid])for(l=0;l<f.data[cid].length;l++)void 0!=f.data[cid][l]&&k.push({i:l,value:f.data[cid][l]});h.selectAll("rect.heatmap.gid"+f.id).data(k).enter().append("rect").attr("class","heatmap hover-visible gid"+f.id).attr("fill",function(a,b){return m(a.value)}).attr("stroke",a.foregroundColor).attr("stroke-width",a.outline+"px").append("title").text(function(a,b){return(f.legend.fields[b]?f.legend.fields[b].name+": ":"")+a.value})}break;
default:console.log("Graph not supported "+f.type)}}ea();J()}else v=0,p.selectAll("g.graph").remove(),p.selectAll("text.legend").remove();C()}function J(){var b=a.nodeHeight,g=H+I;u.selectAll("g.graph").attr("transform",function(b){return"translate("+(a.lineupNodes?phyd3.phylogram.dx-b.y+g:g)+",0)"});var f=p.selectAll("g.graph");d3.select("#graphWidth").attr("value",a.graphWidth);p.selectAll("text.legend").remove();v=0;if(d.graphs){for(var h=0;h<d.graphs.length;h++){var n=d.graphs[h];switch(n.type){case "pie":f.selectAll("path.pie.gid"+
n.id).attr("d",function(a){return d3.svg.arc().innerRadius(0).outerRadius(b)(a)}).attr("transform",function(a){return" translate("+parseInt(a.data.leaf?b+v:0)+",0)"});a.showGraphLegend&&p.append("text").attr("class","legend").text(0!=n.legend.show?n.name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+g+v+2*b)+",-10) rotate(-90)");v+=2*b+5;break;case "binary":f.selectAll("path.binary.gid"+n.id).attr("d",function(a){var d=d3.svg.symbol().size(4*(b-2)*(b-2));d.type(n.legend.fields[a.i].shape);
return d(a)}).attr("transform",function(a){return" translate("+(a.leaf?v+10+a.i*(2*b+5):0)+" ,0)"});if(a.showGraphLegend)for(var k=0;k<n.legend.fields.length;k++)p.append("text").attr("class","legend").text(0!=n.legend.show?n.legend.fields[k].name:"").attr("transform","translate("+(phyd3.phylogram.dx+g+v+k*(2*b+5)+2*b)+", -10) rotate(-90)");v+=(n.legend.fields.length+1)*(2*b+5)+5;break;case "multibar":for(k=0;k<n.legend.fields.length;k++)K[n.id][k]=K[n.id][k].range([0,a.graphWidth]);f.selectAll("rect.multibar.gid"+
n.id).attr("height",parseInt(2*b)).attr("width",function(a,b){return parseInt(K[n.id][a.i]?K[n.id][a.i](a.value):0)}).attr("transform",function(d,e){return" translate("+parseInt(v+d.i*(a.graphWidth+5))+",-"+parseInt(b)+")"});if(a.showGraphLegend)for(k=0;k<n.legend.fields.length;k++)p.append("text").attr("class","legend").text(0!=n.legend.show?n.legend.fields[k].name:"").attr("transform","translate("+(phyd3.phylogram.dx+g+v+k*(a.graphWidth+5)+a.graphWidth/2)+",-10) rotate(-90)");v+=n.legend.fields.length*
(a.graphWidth+5);break;case "heatmap":f.selectAll("rect.heatmap.gid"+n.id).attr("width",2*b).attr("height",2*b).attr("transform",function(a,d){return" translate("+(v+2*a.i*b)+",-"+b+")"});if(a.showGraphLegend)for(k=0;k<n.legend.fields.length;k++)p.append("text").attr("class","legend").text(0!=n.legend.show?n.legend.fields[k].name:"").attr("transform","translate("+parseInt(phyd3.phylogram.dx+g+v+(k+1)*b*2)+",-10) rotate(-90)");v+=2*n.legend.fields.length*b+5}}p.selectAll("text.legend").attr("fill",
e()).attr("font-size",2*a.nodeHeight)}}function ma(){var b=a.nodeHeight;v=0;if(d.graphs)for(var e=0;e<d.graphs.length;e++){var f=d.graphs[e];switch(f.type){case "pie":v+=2*b+5;break;case "binary":v+=(f.legend.fields.length+1)*(2*b+5)+5;break;case "multibar":v+=f.legend.fields.length*(a.graphWidth+5);break;case "heatmap":v+=2*f.legend.fields.length*b+5}}return 2*a.margin+(a.showNodeNames?H:0)+(a.showGraphs?v:0)+(a.showDomains?a.domainWidth:0)}function ea(){var b=[],d=function(d){if(!a.dynamicHide)return"visible";
for(var e=!0,f=a.nodeHeight,g=d.x,l=d.y,h=0;h<b.length;h++){var r=b[h].x,q=b[h].y;if((r-g)*(r-g)+(q-l)*(q-l)<=4*f*f){e=!1;break}}return e?(b.push(d),"visible"):"hidden"};u.selectAll("g.graph").attr("visibility",function(a){return a.show?"visible":"hidden"});p.selectAll("g.inner").selectAll(".graph.pie").attr("visibility",d);p.selectAll("g.inner").selectAll(".graph.binary").attr("visibility",d)}function D(b){phyd3.phylogram.lineAreas=[];phyd3.phylogram.dx=0;var g=d3.layout.cluster().separation(function(a,
b){return 1}).children(a.children||function(a){return a.branchset}).size([L*a.scaleY,E*a.scaleX]);A=g(d);a.showPhylogram||phyd3.phylogram.scaleBranchLengths(A,E*a.scaleX);var f=p.selectAll("path.link");b||(f=f.data(g.links(A)),f.enter().append("svg:path").attr("class","link").attr("fill","none").attr("stroke",e()));f.attr("d",phyd3.phylogram.rightAngleDiagonal);M=void 0;c=0;f=A.filter(function(a){return!a.children});for(g=0;g<f.length;g++)f[g].show=a.dynamicHide?ia(f[g],!a.lineupNodes):!0;M=void 0;
f=A.filter(function(a){return a.children});for(g=0;g<f.length;g++)0<P(f[g]).trim().length?f[g].show=a.dynamicHide?ia(f[g],!0):!0:f[g].show=!1;b||(R=R.data(A),R.enter().append("svg:g").attr("class",function(a){return a.children?0==a.depth?"root node cid_"+a.id:"inner node cid_"+a.id:"leaf node cid_"+a.id}).style("cursor","pointer").on("click",void 0===a.popupAction?ka:a.popupAction).append("rect").attr("class","pointer").attr("fill","red").attr("width",2*a.nodeHeight+2+"px").attr("height",2*a.nodeHeight+
2+"px").style("opacity","0").attr("x","-"+(a.nodeHeight+1)+"px").attr("y","-"+(a.nodeHeight+1)+"px"));R.attr("transform",function(a){return"translate("+parseInt(a.y)+","+parseInt(a.x)+")"});phyd3.phylogram.lineAreas=[]}function N(){D(!0);S()}function ia(b,d){var e=!0,g=b.y+1,h=b.y+5.5*P(b).length,k=b.x-a.nodeHeight,l=b.x+a.nodeHeight;if(!M)return M=b,!0;a.lineupNodes&&!b.children&&(g+=phyd3.phylogram.dx-b.y,h+=phyd3.phylogram.dx-b.y);var q=M,m=q.y+1,r=q.y+5.5*P(q).length,p=q.x-a.nodeHeight,t=q.x+
a.nodeHeight;a.lineupNodes&&!q.children&&(m+=phyd3.phylogram.dx-q.y,r+=phyd3.phylogram.dx-q.y);if(m<=g&&g<=r&&p<=k&&k<=t||m<=h&&h<=r&&p<=k&&k<=t||m<=g&&g<=r&&p<=l&&l<=t||m<=h&&h<=r&&p<=l&&l<=t)e=!1;if(e&&d)for(d=0;d<phyd3.phylogram.lineAreas.length;d++)if(p=phyd3.phylogram.lineAreas[d],q=p.start[0],m=p.end[0],r=p.start[1],p=p.end[1],r==p){if(k<=r&&p<=l&&!(q>h||m<g)){e=!1;break}}else if(q==m&&g<=q&&m<=h&&!(r>l||p<k)){e=!1;break}e&&(M=b);return e}function S(){var b=Y.translate(),d=b[0]+a.marginX-a.translateX,
e=b[1]+a.marginY-a.translateY;p.attr("transform",function(a){return"translate("+parseInt(d)+","+parseInt(e)+") scale(1)"});0!=a.lastScale&&(fa(),a.dynamicHide&&G())}a=a||{};a.scaleY=a.scaleY||1;a.scaleX=a.scaleX||1;a.translateX=a.translateX||0;a.translateY=a.translateY||0;a.height=a.height||800;a.margin=a.margin||20;a.scaleStep=a.scaleStep||.3;a.nodeHeight=a.nodeHeight||6;a.nodeHeightStep=a.nodeHeightStep||1;a.textLength=a.textLength||100;a.domainWidth=a.domainWidth||100;a.domainWidthStep=a.domainWidthStep||
100;a.graphWidth=a.graphWidth||20;a.graphWidthStep=a.graphWidthStep||10;a.domainLevel=a.domainLevel||1;a.domainLevelStep=a.domainLevelStep||10;a.outline=a.outline||.3;a.popupWidth=a.popupWidth||500;a.foregroundColor=a.foregroundColor||"#000";a.backgroundColor=a.backgroundColor||"#fff";a.branchLengthColor=a.branchLengthColor||"red";a.supportValuesColor=a.supportValuesColor||"blue";a.showNodesType=a.showNodesType||"only leaf";a.treeWidth=a.treeWidth||"auto";a.showFullTaxonomy="showFullTaxonomy"in a?
a.showFullTaxonomy:!1;a.showLabels="showLabels"in a?a.showLabels:!0;a.showDomains="showDomains"in a?a.showDomains:!0;a.dynamicHide="dynamicHide"in a?a.dynamicHide:!1;a.invertColors="invertColors"in a?a.invertColors:!1;a.lineupNodes="lineupNodes"in a?a.lineupNodes:!0;a.showSupportValues="showSupportValues"in a?a.showSupportValues:!1;a.showLengthValues="showLengthValues"in a?a.showLengthValues:!1;a.showTaxonomy="showTaxonomy"in a?a.showTaxonomy:!0;a.showTaxonomyColors="showTaxonomyColors"in a?a.showTaxonomyColors:
!0;a.showDomainNames="showDomainNames"in a?a.showDomainNames:!1;a.showDomainColors="showDomainColors"in a?a.showDomainColors:!0;a.showGraphs="showGraps"in a?a.showGraphs:!0;a.showGraphLegend="showGraphLegend"in a?a.showGraphLegend:!0;a.showNodeNames="showNodeNames"in a?a.showNodeNames:!0;a.showPhylogram="showPhylogram"in a?a.showPhylogram:!1;var A,F,M,K=[],I=100,H=100,v=0,O=100,ja=!1;if(d.graphs)for(var Z=0;Z<d.graphs.length;Z++)if(0!=d.graphs[Z].legend.show){ja=!0;break}O=ja?O:0;a.marginX=a.margin;
a.marginY=a.margin+(a.showGraphs?O:0);var U=$(b).width(),E="auto"==a.treeWidth?U-ma():a.treeWidth,L=a.height-(2*a.margin+(a.showGraphs?O:0)),E=parseInt(E),L=parseInt(L);0>E&&(E=U-2*H-(a.showGraphs?2*O:0));var Y=d3.behavior.zoom().scaleExtent([1,1]).on("zoom",function(b){b=0;var d=d3.event.sourceEvent;if(d){if(0<d.deltaY||0>d.wheelDelta)b=-1*a.scaleStep;if(0>d.deltaY||0<d.wheelDelta)b=a.scaleStep;var e=.5,g=.5;d.altKey||d.ctrlKey||(e=d.layerX/U,g=d.layerY/a.height);if(d.ctrlKey||!d.altKey)a.scaleY+=
b,a.scaleY<a.scaleStep?a.scaleY=a.scaleStep:a.translateY+=L*b*g;if(d.altKey||!d.ctrlKey)a.graphWidth+=0==b?0:0>b?-10:10,10>a.graphWidth&&(a.graphWidth=10),a.domainWidth+=0==b?0:0>b?-1*a.domainWidthStep:a.domainWidthStep,a.domainWidth<a.domainWidthStep&&(a.domainWidth=a.domainWidthStep),a.scaleX+=b,a.scaleX<a.scaleStep?a.scaleX=a.scaleStep:a.translateX+=E*b*e;a.lastScale=b;0!=a.lastScale?requestAnimFrame(N):requestAnimFrame(S);d.preventDefault()}});d3.select(b).select("svg").remove();var p=d3.select(b).insert("svg").attr("width",
U+"px").attr("height",a.height+"px").attr("overflow","hidden").attr("version","1.1").attr("font-family","Open Sans").attr("xmlns","http://www.w3.org/2000/svg").call(Y);p.append("svg:rect").attr("class","canvas").attr("width","100%").attr("height","100%").attr("fill",a.invertColors?a.foregroundColor:a.backgroundColor);p.append("svg:text").attr("dy","20px").attr("stroke",e()).attr("stroke-width","1px").attr("font-size","15px").text(d.name?d.name:"");p.append("svg:text").attr("dy","40px").attr("stroke",
e()).attr("stroke-width","0.5px").attr("font-size","10px").text(d.description?d.description:"");var p=p.append("svg:g").attr("id","main").attr("transform","translate("+a.marginX+", "+a.marginY+")"),R=p.selectAll("g.node");D();0==phyd3.phylogram.dx&&(d3.select("#phylogram").attr("checked","").attr("disabled","disabled"),a.showPhylogram=!0,D());var u=p.selectAll("g.leaf.node"),B=u.selectAll("g.domain");aa();ba();m();h();ha();ca();ga();R.append("svg:text").attr("class","name").attr("dx",a.nodeHeight+
